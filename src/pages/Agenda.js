...
import { ChevronLeft, ChevronRight, Plus, X, Pencil, Trash2, Settings, Users, Search } from 'lucide-react'
...
function ModalBloco({ bloco, usuarioId, bancoTarefas, onSave, onClose, onDelete, diaPreSelecionado, horaPreSelecionada, criadorId }) {
  ...
  const [bancoBusca, setBancoBusca] = useState(bloco?.banco_tarefa?.nome || '')
  const [showBancoDrop, setShowBancoDrop] = useState(false)
  ...
  function selecionarBancoTarefa(t) {
    setForm(f => ({ ...f, banco_tarefa_id: t.id, titulo: f.titulo || t.nome }))
    setBancoBusca(t.nome)
    setShowBancoDrop(false)
  }

  function limparBancoTarefa() {
    setForm(f => ({ ...f, banco_tarefa_id: '' }))
    setBancoBusca('')
  }

  const bancoFiltrado = bancoTarefas.filter(t =>
    !bancoBusca || t.nome.toLowerCase().includes(bancoBusca.toLowerCase())
  ).slice(0, 8)
        ...
        <div className="form-grid">
          ...
          {form.tipo === 'tarefa' && (
            <div className="form-group">
              ...
              <div style={{ position: 'relative' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    value={bancoBusca}
                    onChange={e => { setBancoBusca(e.target.value); setShowBancoDrop(true); if (!e.target.value) limparBancoTarefa() }}
                    onFocus={() => setShowBancoDrop(true)}
                    onBlur={() => setTimeout(() => setShowBancoDrop(false), 200)}
                    placeholder="Buscar tarefa do banco..."
                    style={{ paddingLeft: 30, fontSize: 12 }}
                  />
                  {form.banco_tarefa_id && (
                    <button onClick={limparBancoTarefa} style={{ position:'absolute', right:8, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', display:'flex' }}><X size={12}/></button>
                  )}
                </div>
                {showBancoDrop && (
                  <div style={{ position:'absolute', top:'calc(100% + 4px)', left:0, right:0, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, boxShadow:'0 4px 16px rgba(0,0,0,0.15)', zIndex:50, maxHeight:200, overflowY:'auto' }}>
                    {bancoFiltrado.length === 0
                      ? <div style={{ padding:'10px 14px', fontSize:12, color:'var(--text-muted)' }}>Nenhuma tarefa encontrada</div>
                      : bancoFiltrado.map(t => (
                        <button key={t.id} onMouseDown={() => selecionarBancoTarefa(t)} style={{ width:'100%', padding:'8px 12px', textAlign:'left', background: form.banco_tarefa_id === t.id ? 'var(--accent-glow)' : 'transparent', border:'none', cursor:'pointer', display:'flex', flexDirection:'column', gap:2, borderBottom:'1px solid var(--border)' }} onMouseEnter={e=>e.currentTarget.style.background='var(--surface-2)'} onMouseLeave={e=>e.currentTarget.style.background=form.banco_tarefa_id===t.id?'var(--accent-glow)':'transparent'}>
                          <span style={{ fontSize:12, fontWeight:600, color: form.banco_tarefa_id === t.id ? 'var(--accent)' : 'var(--text)' }}>{t.nome}</span>
                          {t.categoria && <span style={{ fontSize:10, color:'var(--text-muted)' }}>{t.categoria.nome}</span>}
                        </button>
                      ))
                    }
                  </div>
              ...
              {form.banco_tarefa_id && <div style={{ fontSize:11, color:'var(--accent)', marginTop:3 }}>✓ Tarefa do banco vinculada — o título será preenchido automaticamente se estiver vazio</div>}
            </div>
          )}
