          ...
          <div style={{ minWidth: 700 }}>
            ...
            <div style={{ border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden' }}>
              {slots.map((hora, si) => {
                ...
                return (
                  <div key={hora} style={{ display: 'grid', gridTemplateColumns: '64px repeat(5, 1fr)', borderBottom: si < slots.length - 1 ? '1px solid var(--border)' : 'none', minHeight: 40, background: almoco ? 'rgba(99,102,241,0.04)' : si % 2 === 0 ? 'var(--surface)' : 'var(--surface)' }}>
                    {/* Hora */}
                    ...
                    {Array.from({ length: 5 }, (_, di) => {
                      ...
                      return (
                        <div key={di} style={{ borderRight: di < 4 ? '1px solid var(--border)' : 'none', padding: '2px 4px', minHeight: 40, background: isHojeCol ? 'rgba(99,102,241,0.02)' : 'transparent', position: 'relative', cursor: podeEditar && !almoco ? 'pointer' : 'default' }}
                          onClick={() => {
                            if (!podeEditar || almoco) return
                            setDiaPreSel(diaSemana)
                            setHoraPreSel(hora)
                            setModalBloco('new')
                          }}
                        >
                          {almoco && si === slots.findIndex(s => s === config.hora_almoco) && (
                            <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, color: 'var(--text-muted)', fontStyle: 'italic', pointerEvents: 'none' }}>
                              {di === 2 ? `Almoço (${config.dur_almoco}min)` : ''}
                            </div>
                          )}
                          {blocosAqui.map(b => {
