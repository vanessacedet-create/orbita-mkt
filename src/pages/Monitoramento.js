...
function diffDias(a,b){ // a - b em dias (keys YYYY-MM-DD)
  return Math.round((new Date(a+'T12:00:00')-new Date(b+'T12:00:00'))/86400000)
}
function fmtCurta(key){
...
// Status derivado (não existe no banco): pendente com data já vencida
const STATUS_ATRASADO = {value:'atrasado', label:'Atrasado', cor:'#f97316', icon:'⚠️'}

function statusInfo(s){
  if(s==='atrasado')return STATUS_ATRASADO
  return STATUS_OPTS.find(x=>x.value===s)||STATUS_OPTS[0]
}
// Status efetivo: 'pendente' vencido vira 'atrasado' (calculado, nada gravado no banco)
function statusEfetivo(r, hj){
  if(r.status==='pendente'&&r.data&&r.data<hj)return'atrasado'
  return r.status
}
...
// ── PAINEL AÇÕES DE HOJE ───────────────────────────────────
// Derivado dos dados já carregados — nenhuma tabela nova.
...
function AcoesHoje({todos, hj, onClickDia}){
  ...
  const atrasados = todos.filter(r=>
    r.data&&statusEfetivo(r,hj)==='atrasado'
  ).sort((a,b)=>a.data.localeCompare(b.data))

  const semRetorno = todos.filter(r=>r.status==='sem_retorno')
    .sort((a,b)=>(a.data||'').localeCompare(b.data||''))

  const total = confirmar.length+atrasados.length+semRetorno.length
  const[aberto,setAberto]=useState(true)

  if(total===0)return(
    <div style={{display:'flex',alignItems:'center',gap:10,background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'12px 18px',marginBottom:16}}>
      <BellRing size={16} color="#22c55e"/>
      <span style={{fontSize:13,color:'var(--text-muted)'}}>Nenhuma ação pendente neste período. Tudo em dia ✅</span>
    </div>
  )
      ...
      <div style={{flex:'1 1 240px',minWidth:220}}>
        ...
        {itens.slice(0,5).map((r,i)=>(
          <div key={r.id||r._lancamentoId||i} onClick={()=>r.data&&onClickDia(r.data)}
            ...
            {r.data&&(
              <span style={{fontSize:10,fontWeight:700,color:cor,whiteSpace:'nowrap'}}>
                {r.data===hj?'hoje':r.data<hj?`${diffDias(hj,r.data)}d atrás`:fmtCurta(r.data)}
              </span>
            )}
          </div>
        ))}
  ...
  return(
    <div style={{background:'var(--surface)',border:'1px solid var(--border)',borderRadius:12,padding:'14px 18px',marginBottom:16}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer'}} onClick={()=>setAberto(a=>!a)}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          <BellRing size={16} color="var(--accent)"/>
          <span style={{fontSize:13,fontWeight:800,color:'var(--text)'}}>Ações de hoje</span>
          <span style={{fontSize:11,fontWeight:800,color:'#fff',background:'var(--accent)',borderRadius:10,padding:'0 8px',lineHeight:'18px'}}>{total}</span>
        </div>
        <span style={{fontSize:11,color:'var(--text-muted)'}}>{aberto?'ocultar':'mostrar'}</span>
      </div>
      {aberto&&(
        <div style={{display:'flex',gap:18,flexWrap:'wrap',marginTop:12}}>
          <Grupo titulo="Confirmar divulgação" cor="#3b82f6" icone={<CalendarClock size={13} color="#3b82f6"/>}
            itens={confirmar} acao="Combinar/confirmar com o parceiro"/>
          <Grupo titulo="Atrasados — cobrar" cor="#f97316" icone={<AlertTriangle size={13} color="#f97316"/>}
            itens={atrasados} acao="Data combinada vencida sem publicação"/>
          <Grupo titulo="Sem retorno — insistir" cor="#eab308" icone={<MessageCircle size={13} color="#eab308"/>}
            itens={semRetorno} acao="Parceiro não respondeu à última cobrança"/>
        </div>
      )}
    </div>
  )
    ...
    <div className="modal-backdrop" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{maxWidth:500,maxHeight:'80vh',overflowY:'auto'}}>
        ...
        {registros.length===0
          ...
          :registros.map(r=>{
            const st=statusInfo(statusEfetivo(r,hj))
      ...
      {semanas.map((sem,si)=>(
        <div key={si} style={{display:'grid',gridTemplateColumns:'repeat(7,minmax(0,1fr))',borderBottom:si===semanas.length-1?'none':'1px solid var(--border)'}}>
          {sem.map(({key,dia,doMes},ci)=>{
            ...
            const ehH = key===hj
            const fds = ci===0||ci===6
            // Contagem por status (efetivo — pendente vencido conta como atrasado)
            const postou = regs.filter(r=>statusEfetivo(r,hj)==='postou').length
            const naoPst = regs.filter(r=>statusEfetivo(r,hj)==='nao_postou').length
            const semRet = regs.filter(r=>statusEfetivo(r,hj)==='sem_retorno').length
            const pendente = regs.filter(r=>statusEfetivo(r,hj)==='pendente').length
            const atrasado = regs.filter(r=>statusEfetivo(r,hj)==='atrasado').length
            return(
              <div key={key} onClick={()=>doMes&&onClickDia(key)}
                ...
                {regs.length>0&&(
                  <div style={{display:'flex',flexWrap:'wrap',gap:3}}>
                    ...
                    {atrasado>0&&<div style={{background:'#f9731622',border:'1px solid #f9731644',borderRadius:4,padding:'1px 5px',fontSize:10,color:'#f97316',fontWeight:700}}>⚠️{atrasado}</div>}
                ...
                {/* Nomes (até 2) */}
                {regs.slice(0,2).map((r,ri)=>{
                  const st=statusInfo(statusEfetivo(r,hj))
                  return(
                    <div key={r.id||r._lancamentoId||ri} style={{marginTop:2,fontSize:10,color:'var(--text)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',
...
export default function Monitoramento(){
  ...
  // ── União: registros manuais + lançamentos com data_combinada ──
  // "todos" alimenta o painel de ações, os totais e o calendário.
  const todos = []
  for(const r of registros){
    if(!r.data)continue
    todos.push({...r, _origem:'manual'})
  }
  for(const lp of lancamentos){
    if(!lp.data_combinada)continue
    if(!todos.find(x=>x._lancamentoId===lp.id)){
      todos.push({
        _lancamentoId: lp.id,
        _origem: 'lancamento',
  ...
  // ── Totais unificados (manuais + lançamentos, com atrasado derivado) ──
  const totalPostou = todos.filter(r=>statusEfetivo(r,hj)==='postou').length
  const totalNao = todos.filter(r=>statusEfetivo(r,hj)==='nao_postou').length
  const totalAtras = todos.filter(r=>statusEfetivo(r,hj)==='atrasado').length
  const totalPend = todos.filter(r=>statusEfetivo(r,hj)==='pendente').length
  const totalSemRet = todos.filter(r=>statusEfetivo(r,hj)==='sem_retorno').length
    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12}}>
        <div style={{display:'flex',alignItems:'center',gap:14}}>
          ...
          <div>
            ...
            <p style={{fontSize:12,color:'var(--text-muted)',margin:0}}>
              {todos.length} registro{todos.length!==1?'s':''} no período
            </p>
          </div>
        </div>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16,flexWrap:'wrap'}}>
          ...
          {/* Resumo — agora inclui lançamentos e o status derivado Atrasado */}
          <div style={{display:'flex',gap:16}}>
            {[{n:totalPostou,l:'Postaram',c:'#22c55e'},{n:totalNao,l:'Não postaram',c:'#ef4444'},{n:totalAtras,l:'Atrasados',c:'#f97316'},{n:totalSemRet,l:'Sem retorno',c:'#eab308'},{n:totalPend,l:'Pendentes',c:'#6b7280'}].map(({n,l,c})=>(
      ...
      <div style={{display:'flex',gap:16,marginTop:12,flexWrap:'wrap'}}>
        {[{c:'#22c55e',l:'✅ Postou'},{c:'#ef4444',l:'❌ Não postou'},{c:'#f97316',l:'⚠️ Atrasado (data vencida)'},{c:'#eab308',l:'💬 Sem retorno'},{c:'#6b7280',l:'🕐 Pendente'}].map(({c,l})=>(
