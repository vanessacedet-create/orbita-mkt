...
const getGrupos        = async () => (await supabase.from('rh_parceiras_grupos').select('*').order('nome')).data || []
const getColaboradores = async () => (await supabase.from('rh_parceiras_colaboradores').select('*, rh_parceiras_grupos(id,nome)').order('nome')).data || []
const getTodasAusencias= async () => (await supabase.from('rh_parceiras_ausencias').select('*, rh_parceiras_colaboradores(id,nome,rh_parceiras_grupos(nome))').order('data_inicio')).data || []
const getAusencias     = async (id) => (await supabase.from('rh_parceiras_ausencias').select('*').eq('colaborador_id',id).order('data_inicio',{ascending:false})).data || []
const getFeedbacks     = async (id) => (await supabase.from('rh_parceiras_feedbacks').select('*').eq('colaborador_id',id).order('data',{ascending:false})).data || []

async function saveGrupo(g) {
  const p = {nome:g.nome,descricao:g.descricao,responsavel:g.responsavel}
  if (g.id) return (await supabase.from('rh_parceiras_grupos').update(p).eq('id',g.id).select().single()).data
  return (await supabase.from('rh_parceiras_grupos').insert([p]).select().single()).data
}
async function deleteGrupo(id) { await supabase.from('rh_parceiras_grupos').delete().eq('id',id) }
...
async function deleteColaborador(id) { await supabase.from('rh_parceiras_colaboradores').delete().eq('id',id) }

async function getTarefasAvaliacao(colaborador_id) {
  const { data } = await supabase.from('rh_parceiras_tarefas_avaliacao')
...
async function saveTarefaAvaliacao(t) {
  ...
  if (t.id) return (await supabase.from('rh_parceiras_tarefas_avaliacao').update(p).eq('id',t.id).select('*,supervisor:supervisor_id(id,nome)').single()).data
  return (await supabase.from('rh_parceiras_tarefas_avaliacao').insert([p]).select('*,supervisor:supervisor_id(id,nome)').single()).data
}
async function deleteTarefaAvaliacao(id) { await supabase.from('rh_parceiras_tarefas_avaliacao').delete().eq('id',id) }
async function importarTarefasAvaliacao(colaborador_id, rows) {
  ...
  const { data, error } = await supabase.from('rh_parceiras_tarefas_avaliacao').insert(inserts).select('*,supervisor:supervisor_id(id,nome)')
...
async function saveAusencia(a) {
  ...
  if (a.id) return (await supabase.from('rh_parceiras_ausencias').update(p).eq('id',a.id).select().single()).data
  return (await supabase.from('rh_parceiras_ausencias').insert([p]).select().single()).data
}
async function deleteAusencia(id) { await supabase.from('rh_parceiras_ausencias').delete().eq('id',id) }
...
async function saveFeedback(f) {
  ...
  if (f.id) return (await supabase.from('rh_parceiras_feedbacks').update(p).eq('id',f.id).select().single()).data
  return (await supabase.from('rh_parceiras_feedbacks').insert([p]).select().single()).data
}
async function deleteFeedback(id) { await supabase.from('rh_parceiras_feedbacks').delete().eq('id',id) }

async function getAvaliacoes(colaborador_id) {
  const { data } = await supabase.from('rh_parceiras_avaliacoes')
    .select('*, rh_parceiras_criterios_avaliacao(*)')
    .eq('colaborador_id', colaborador_id)
    .order('created_at', {ascending:false})
  return data || []
}
async function getTodasAvaliacoes() {
  const { data } = await supabase.from('rh_parceiras_avaliacoes')
    .select('*, rh_parceiras_colaboradores(id,nome,rh_parceiras_grupos(nome)), rh_parceiras_criterios_avaliacao(*)')
    .order('created_at', {ascending:false})
  return data || []
}
async function saveAvaliacao(av, criterios) {
  ...
  let avId = av.id
  if (avId) {
    await supabase.from('rh_parceiras_avaliacoes').update(payload).eq('id',avId)
  } else {
    const { data } = await supabase.from('rh_parceiras_avaliacoes').insert([payload]).select().single()
    avId = data.id
  }
  await supabase.from('rh_parceiras_criterios_avaliacao').delete().eq('avaliacao_id',avId)
  if (criterios.length>0) {
    await supabase.from('rh_parceiras_criterios_avaliacao').insert(
      criterios.map(c=>({avaliacao_id:avId,nome:c.nome,nota:Number(c.nota),peso:Number(c.peso)||1}))
    )
  }
  const { data } = await supabase.from('rh_parceiras_avaliacoes').select('*,rh_parceiras_criterios_avaliacao(*)').eq('id',avId).single()
  return data
}
async function deleteAvaliacao(id) {
  await supabase.from('rh_parceiras_avaliacoes').update({comentarios:'[ARQUIVADA] '}).eq('id',id)
}

async function getOKRs() {
  const { data } = await supabase.from('rh_parceiras_okrs')
    .select('*, rh_parceiras_colaboradores(id,nome), rh_parceiras_grupos(id,nome), rh_parceiras_key_results(*)')
    .order('created_at',{ascending:false})
  return data || []
}
async function saveOKR(okr) {
  ...
  if (okr.id) return (await supabase.from('rh_parceiras_okrs').update(p).eq('id',okr.id).select('*, rh_parceiras_colaboradores(id,nome), rh_parceiras_grupos(id,nome), rh_parceiras_key_results(*)').single()).data
  return (await supabase.from('rh_parceiras_okrs').insert([p]).select('*, rh_parceiras_colaboradores(id,nome), rh_parceiras_grupos(id,nome), rh_parceiras_key_results(*)').single()).data
}
async function deleteOKR(id) { await supabase.from('rh_parceiras_okrs').delete().eq('id',id) }
...
async function saveKR(kr) {
  ...
  if (kr.id) return (await supabase.from('rh_parceiras_key_results').update(p).eq('id',kr.id).select().single()).data
  return (await supabase.from('rh_parceiras_key_results').insert([p]).select().single()).data
}
async function deleteKR(id) { await supabase.from('rh_parceiras_key_results').delete().eq('id',id) }
async function updateKRProgresso(id, valor_atual) {
  return (await supabase.from('rh_parceiras_key_results').update({valor_atual:Number(valor_atual)}).eq('id',id).select().single()).data
}

async function getTreinamentos() {
  const { data } = await supabase.from('rh_parceiras_treinamentos').select('*, rh_parceiras_grupos(id,nome)').order('titulo')
  return data || []
}
async function saveTreinamento(t) {
  ...
  if (t.id) return (await supabase.from('rh_parceiras_treinamentos').update(p).eq('id',t.id).select('*,rh_parceiras_grupos(id,nome)').single()).data
  return (await supabase.from('rh_parceiras_treinamentos').insert([p]).select('*,rh_parceiras_grupos(id,nome)').single()).data
}
async function deleteTreinamento(id) { await supabase.from('rh_parceiras_treinamentos').delete().eq('id',id) }

async function getProgressoColab(colaborador_id) {
  const { data } = await supabase.from('rh_parceiras_progresso_treinamento')
    .select('*, rh_parceiras_treinamentos(*)')
    .eq('colaborador_id', colaborador_id)
  return data || []
}
async function getProgressoTreinamento(treinamento_id) {
  const { data } = await supabase.from('rh_parceiras_progresso_treinamento')
    .select('*, rh_parceiras_colaboradores(id,nome)')
...
async function upsertProgresso(colaborador_id, treinamento_id, status, data_conclusao) {
  ...
  const { data } = await supabase.from('rh_parceiras_progresso_treinamento')
    ...
    .select('*,rh_parceiras_treinamentos(*)').single()
  return data
}

async function getProcessos(colaborador_id) {
  const { data } = await supabase.from('rh_parceiras_processos')
    .select('*, rh_parceiras_checklist_itens(*)').eq('colaborador_id', colaborador_id).order('created_at',{ascending:false})
  return data || []
}
async function createProcesso(colaborador_id, tipo, responsavel) {
  ...
  const { data: proc } = await supabase.from('rh_parceiras_processos')
    .insert([{colaborador_id,tipo,responsavel:responsavel||null,data_inicio:new Date().toISOString().slice(0,10)}])
    .select().single()
  if (itens.length) {
    await supabase.from('rh_parceiras_checklist_itens').insert(
      itens.map((desc,i)=>({processo_id:proc.id,descricao:desc,concluido:false,ordem:i}))
    )
  }
  const { data } = await supabase.from('rh_parceiras_processos').select('*,rh_parceiras_checklist_itens(*)').eq('id',proc.id).single()
  return data
}
async function toggleChecklistItem(id, concluido) {
  const { data } = await supabase.from('rh_parceiras_checklist_itens').update({concluido}).eq('id',id).select().single()
  return data
}
async function addChecklistItem(processo_id, descricao) {
  const { data } = await supabase.from('rh_parceiras_checklist_itens').insert([{processo_id,descricao,concluido:false,ordem:999}]).select().single()
  return data
}
async function deleteChecklistItem(id) { await supabase.from('rh_parceiras_checklist_itens').delete().eq('id',id) }
async function concludeProcesso(id) {
  await supabase.from('rh_parceiras_processos').update({data_conclusao:new Date().toISOString().slice(0,10)}).eq('id',id)
}

async function getTodosProgressosTreinamento() {
  const { data } = await supabase.from('rh_parceiras_progresso_treinamento')
    .select('*, rh_parceiras_colaboradores(id,nome,cargo,rh_parceiras_grupos(nome)), rh_parceiras_treinamentos(id,titulo,tipo,obrigatorio)')
...
function ModalAvaliacao({ avaliacao, colaborador_id, onSave, onClose }) {
  ...
  const [criterios, setCriterios] = useState(
    avaliacao?.rh_parceiras_criterios_avaliacao?.length
      ? avaliacao.rh_parceiras_criterios_avaliacao.map(c=>({...c,nota:String(c.nota),peso:String(c.peso)}))
                    ...
                    <div key={av.id} className="table-card" style={{padding:'14px 18px'}}>
                      ...
                      {exp&&(
                        <div style={{marginTop:14,paddingTop:14,borderTop:'1px solid var(--border)'}}>
                          {av.rh_parceiras_criterios_avaliacao?.length>0&&(
                            <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:12}}>
                              {av.rh_parceiras_criterios_avaliacao.map(c=>(
...
function CardOKR({ okr, onEdit, onDelete, onEditKR, onDeleteKR, onUpdateProgresso }) {
  ...
  const krs = okr.rh_parceiras_key_results || []
      ...
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10}}>
        <div style={{flex:1,cursor:'pointer'}} onClick={()=>setExpanded(p=>!p)}>
          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:4}}>
            ...
            <span style={{fontSize:11,color:'var(--text-muted)'}}>{okr.tipo==='grupo'?`📦 ${okr.rh_parceiras_grupos?.nome||'—'}`:`👤 ${okr.rh_parceiras_colaboradores?.nome||'—'}`}</span>
          </div>
          ...
          {ausenciasAll.filter(a=>a.data_fim>=hoje&&['planejado','aprovado'].includes(a.status)).length===0
            ...
            : ausenciasAll.filter(a=>a.data_fim>=hoje&&['planejado','aprovado'].includes(a.status)).slice(0,5).map(a=>(
              <div key={a.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8,fontSize:12}}>
                <span style={{fontWeight:600,color:'var(--text)'}}>{a.rh_parceiras_colaboradores?.nome||'—'}</span>
                  ...
                  <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8}}>
                    <div style={{flex:1}}>
                      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:4}}>
                        ...
                        {t.rh_parceiras_grupos && <span style={{fontSize:10,color:'var(--text-muted)',background:'var(--surface-2)',border:'1px solid var(--border)',borderRadius:4,padding:'1px 6px'}}>{t.rh_parceiras_grupos.nome}</span>}
                      </div>
...
function CardProcesso({ processo, onUpdate }) {
  const [itens, setItens]     = useState(processo.rh_parceiras_checklist_itens||[])
...
function TreinamentosColaborador({ colaborador_id, showToast }) {
  ...
  const todos = [
    ...progressos.map(p=>({...p.rh_parceiras_treinamentos, _status:p.status, _prog:p})),
...
export default function RHParceiras() {
  ...
  const [modoAusencia, setModoAusencia] = useState('ciclo')
  const [viewAusencia, setViewAusencia]   = useState('lista')
  ...
  async function handleSaveKR(kr) {
    ...
    setOKRs(p=>p.map(o=>o.id===kr.okr_id?{...o,rh_parceiras_key_results:kr.id?(o.rh_parceiras_key_results||[]).map(x=>x.id===kr.id?s:x):[...(o.rh_parceiras_key_results||[]),s]}:o))
  ...
  async function handleDelKR(id, okr_id) {
    ...
    await deleteKR(id)
    setOKRs(p=>p.map(o=>o.id===okr_id?{...o,rh_parceiras_key_results:(o.rh_parceiras_key_results||[]).filter(x=>x.id!==id)}:o))
    showToast('Removido!')
  }
  async function handleUpdateProgresso(id, valor, okr_id) {
    const s=await updateKRProgresso(id,valor)
    setOKRs(p=>p.map(o=>o.id===okr_id?{...o,rh_parceiras_key_results:(o.rh_parceiras_key_results||[]).map(x=>x.id===id?s:x)}:o))
    showToast('Progresso atualizado!')
  }
  ...
  function handleExportAusencias(formato) {
    ...
    const linhas = [cabecalho, ...aus.map(a=>{
      ...
      return [
        a.rh_parceiras_colaboradores?.nome || colab?.nome || '—',
        colab?.cargo || '—',
        a.rh_parceiras_colaboradores?.rh_parceiras_grupos?.nome || '—',
        ta?.l || a.tipo,
  ...
  function handleExportOKRs(formato) {
    ...
    const linhas = [cabecalho, ...okrsFiltrados.map(okr=>{
      ...
      const responsavel = okr.tipo === 'individual'
        ? (okr.rh_parceiras_colaboradores?.nome || '—')
        : (okr.rh_parceiras_grupos?.nome || '—')
  ...
  async function handleExportTreinamentos(formato) {
    ...
    const linhas = [cabecalho, ...progressos.map(p=>[
      p.rh_parceiras_colaboradores?.nome || '—',
      p.rh_parceiras_colaboradores?.cargo || '—',
      p.rh_parceiras_colaboradores?.rh_parceiras_grupos?.nome || '—',
      p.rh_parceiras_treinamentos?.titulo || '—',
      p.rh_parceiras_treinamentos?.tipo || '—',
      p.rh_parceiras_treinamentos?.obrigatorio ? 'Sim' : 'Não',
              ...
              ausDoPeriodo.length===0
                ...
                :<div className="table-card"><table>
                  ...
                  <tbody>{ausDoPeriodo.map(a=>{
                    const ta=TIPO_AUSENCIA.find(t=>t.v===a.tipo); const sa=STATUS_AUSENCIA.find(s=>s.v===a.status)||STATUS_AUSENCIA[0]
                    return <tr key={a.id}>
                      <td className="td-strong" style={{cursor:'pointer'}} onClick={()=>setPerfil(colaboradores.find(c=>c.id===a.colaborador_id))}>{a.rh_parceiras_colaboradores?.nome||'—'}</td>
                      <td style={{fontSize:12,color:'var(--text-muted)'}}>{a.rh_parceiras_colaboradores?.rh_parceiras_grupos?.nome||'—'}</td>
                      ...
                      return (
                        <div key={dia} style={{
                          ...
                          {ausencias.slice(0,3).map(a=>{
                            const nome = a.rh_parceiras_colaboradores?.nome?.split(' ')[0] || '—'
                            const cor  = COR_TIPO[a.tipo] || 'var(--accent)'
                            return (
                              <div key={a.id} title={`${a.rh_parceiras_colaboradores?.nome} — ${TIPO_AUSENCIA.find(t=>t.v===a.tipo)?.l}`}
                                style={{
