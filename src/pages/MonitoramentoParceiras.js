...
function ViewChecklistParceiras({ livrarias, checkagemMes, formato, dataKey, onMarcar, onGerarDia }) {
  ...
  const semRegistro = livrarias.filter(l => !mapa[l.editora_id || l.id])
  ...
  return (
    <div>
      {semRegistro.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 10, padding: '10px 16px', marginBottom: 12, gap: 12 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {livrarias.map(livraria => {
          const chaveId = livraria.editora_id || livraria.id
          const reg = mapa[chaveId]
          const status = reg?.status || null
          return (
            <div key={livraria.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8 }}>
              <div style={{ minWidth: 200, maxWidth: 260 }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 3, marginTop: 1 }}>
                  <Instagram size={9} />
                  {livraria.instagram
                    ? <a href={`https://instagram.com/${livraria.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" style={{ color: 'var(--accent)' }}>{livraria.instagram}</a>
              ...
              <div style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
                {STATUS_PARCEIRAS.map(s => (
                  <button key={s.value} onClick={() => onMarcar({ editora: { id: chaveId }, formato, dataKey, status: s.value })}
                    style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `2px solid ${s.cor}`, background: status === s.value ? s.cor : 'transparent', color: status === s.value ? '#fff' : s.cor, transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
                    {s.label}
                  </button>
                ))}
              </div>
        ...
        return (
          <div key={livraria.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 14px', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8 }}>
            ...
            <select value={responsavel}
              ...
              style={{ padding: '4px 8px', borderRadius: 8, fontSize: 12, border: '1px solid var(--border)', background: responsavel ? 'var(--accent-glow)' : 'var(--surface-2)', color: responsavel ? 'var(--accent)' : 'var(--text-muted)', fontWeight: responsavel ? 700 : 400, cursor: 'pointer', minWidth: 130 }}>
              <option value="">Responsável...</option>
              {EQUIPE.map(n => <option key={n} value={n}>{n}</option>)}
            </select>
            <div style={{ display: 'flex', gap: 6, marginLeft: 'auto' }}>
              {STATUS_CRIATIVO.map(s => (
                <button key={s.value} onClick={() => onMarcar({ editora: { id: chaveId }, formato, dataKey, status: s.value, responsavel })}
                  style={{ padding: '4px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: `2px solid ${s.cor}`, background: status === s.value ? s.cor : 'transparent', color: status === s.value ? '#fff' : s.cor, transition: 'all 0.15s', whiteSpace: 'nowrap' }}>
                  {s.label}
                </button>
              ))}
...
export default function MonitoramentoParceiras() {
  ...
  async function carregarDados() {
    try {
      ...
      setEditoras(eds)
      setLivrarias(livs) // todas, sem filtrar por editora_id
