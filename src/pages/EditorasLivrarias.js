...
function ModalEditora({ editora, onSave, onClose }) {
  const empty = {
    ...
    sub_nicho: '', posicionamento: '', grupo_id: '', status_parceria: 'ativa', selos: [],
  }
  const [form, setForm] = useState(editora ? {
    ...
    posicionamento: editora.posicionamento ?? '',
  } : empty)
    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth: 560, maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <div className="form-grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          ...
          {input('Posicionamento', 'posicionamento')}
...
function ModalLivraria({ livraria, editoras, onSave, onClose }) {
  ...
  const [form, setForm] = useState(livraria ? { ...livraria, editora_id: livraria.editora_id ?? '' } : empty)
  const [saving, setSaving] = useState(false)
  ...
  function baixarTemplate() {
    ...
    if (tipo === 'editora') {
      const ws = XLSX.utils.aoa_to_sheet([
        ['Nome','Contato','Email','Instagram','YouTube','Site','Seguidores','Canal de venda','Tem grupo (Sim/Não)','Macro','Nicho','Sub-nicho','Posicionamento','Grupo ID (1-8)','Status (ativa/encerramento/finalizada/pendente)'],
        ['Editora Exemplo','Paulo Silva','paulo@ex.com','@editora','','https://editora.com','','Instagram','Sim','Catolicismo','Doutrina','','Conservador','1','ativa'],
      ])
      ws['!cols'] = [30,16,24,16,16,24,12,16,18,14,14,14,16,12,16].map(w => ({ wch: w }))
      ...
      ws['!cols'] = [24,16,24,16,24,24].map(w => ({ wch: w }))
    ...
    reader.onload = e => {
      ...
      if (tipo === 'editora') {
        setLinhas(dados.map(r => ({
          ...
          macro: r[9], nicho: r[10], sub_nicho: r[11], posicionamento: r[12],
          grupo_id: r[13], status_parceria: r[14],
        })))
      } else {
        setLinhas(dados.map(r => {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: '2px solid var(--border)' }}>
              {['Class.','Nome','Contato','E-mail','Instagram','Site','Seguidores','Canal','Macro','Nicho','Posicionamento','Grupo','Tem grupo?','Status','Selos',''].map(h => (
          ...
          <tbody>
            {lista.length === 0 ? (
              <tr><td colSpan={16} style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)', fontSize: 13 }}>Nenhuma editora encontrada.</td></tr>
            ) : lista.map(e => (
              <tr key={e.id} style={{ borderBottom: '1px solid var(--border)' }}
                ...
                <td style={{ padding: '10px 10px', color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{val(e.posicionamento)}</td>
                <td style={{ padding: '10px 10px', whiteSpace: 'nowrap' }}>
                  {e.grupo_id
...
function AbaGrupos({ editoras, livrarias }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
      {GRUPOS.map(grupo => {
        const editorasDoGrupo = editoras.filter(e => e.grupo_id === grupo.id)
        // Livrarias cujas editoras estão neste grupo
        const livrariasDoGrupo = livrarias.filter(l =>
          l.editora_id && editorasDoGrupo.some(e => e.id === l.editora_id)
        )
        const total = editorasDoGrupo.length + livrariasDoGrupo.length
