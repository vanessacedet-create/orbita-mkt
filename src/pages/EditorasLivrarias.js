...
import { BookOpen, Plus, X, Upload, Pencil, Trash2, FileSpreadsheet, Building2, Library, LayoutGrid, Settings2, CheckSquare } from 'lucide-react'
...
const COLS_EDITORAS_DEFAULT = ['classificacao','nome','status_parceria','macro','posicionamento','grupo_id']
...
function getStatusSafe(status) {
  return STATUS_COR[status] || STATUS_COR['ativa']
}
    ...
    <div style={{ position:'absolute', top:36, right:0, background:'var(--surface)', border:'1px solid var(--border)', borderRadius:10, padding:12, zIndex:50, minWidth:220, boxShadow:'0 8px 24px rgba(0,0,0,0.2)' }}>
      ...
      {colunas.filter(c => !c.fixo).map(c => (
        <label key={c.key} style={{ display:'flex', alignItems:'center', gap:8, padding:'4px 0', cursor:'pointer', fontSize:13 }}>
          <input type="checkbox" checked={visiveis.includes(c.key)}
            onChange={e => e.target.checked ? onChange([...visiveis, c.key]) : onChange(visiveis.filter(k => k !== c.key))}
...
function BarraSeleção({ selecionados, total, onSelecionarTodos, onDeselecionarTodos, onExcluir, excluindo }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:12, padding:'8px 14px', background:'var(--accent-glow)', border:'1px solid var(--accent)', borderRadius:8, marginBottom:12 }}>
      ...
      <button className="btn btn-sm" onClick={onExcluir} disabled={excluindo}
        style={{ marginLeft:'auto', background:'#ef4444', color:'#fff', border:'none', borderRadius:6, padding:'5px 14px', cursor:'pointer', fontSize:12, fontWeight:600 }}>
...
function AbaEditoras({ editoras, livrarias, setEditoras, isAdmin, showToast }) {
  ...
  function renderCelula(e, key) {
    if (key === 'classificacao') return e.classificacao
      ? <span style={{ fontWeight:800, color:CLASS_COR[e.classificacao] || 'var(--accent)' }}>{e.classificacao}</span> : null
    if (key === 'nome') {
  ...
  return (
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <div style={{ position:'relative' }}>
            ...
            {showColSel && <SeletorColunas colunas={COLUNAS_EDITORAS} visiveis={colsVisiveis} onChange={setColsVisiveis} />}
          </div>
          {isAdmin && <>
      ...
      <BarraSeleção
        selecionados={selecionados.size} total={lista.length}
            ...
            {lista.length === 0 ? (
              ...
              return (
                <tr key={e.id}
                  ...
                  {colsAtivas.map(c => (
                    <Celula key={c.key} width={c.key === 'nome' ? 220 : c.key === 'grupo_id' ? 60 : 140}>
...
function AbaLivrarias({ livrarias, setLivrarias, editoras, isAdmin, showToast }) {
  ...
  function renderCelula(l, key) {
    ...
    if (key === 'instagram' && l.instagram)
      return <a href={`https://instagram.com/${l.instagram.replace('@','')}`} target="_blank" rel="noreferrer" style={{ color:'var(--accent)' }}>{l.instagram}</a>
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12, flexWrap:'wrap', gap:10 }}>
        ...
        <div style={{ display:'flex', gap:8, alignItems:'center' }}>
          <div style={{ position:'relative' }}>
            <button className="btn btn-ghost btn-sm" onClick={() => setShowColSel(v => !v)}><Settings2 size={13} /> Colunas</button>
            {showColSel && <SeletorColunas colunas={COLUNAS_LIVRARIAS} visiveis={colsVisiveis} onChange={setColsVisiveis} />}
          </div>
          {isAdmin && <>
            <button className="btn btn-ghost btn-sm" onClick={() => setImportar(true)}><Upload size={13} /> Importar</button>
