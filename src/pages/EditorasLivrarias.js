...
const TODAS_COLUNAS_LIVRARIAS = [
  ...
  { key: 'site',        label: 'Site' },
  { key: 'instagram',   label: 'Instagram' },
  { key: 'instagram2',  label: 'Instagram 2' },
  { key: 'youtube',     label: 'YouTube' },
    ...
    <div className="modal-backdrop" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ maxWidth:500, maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div className="form-grid" style={{ gridTemplateColumns:'1fr 1fr' }}>
          ...
          <div className="form-group"><label className="form-label">Instagram 2</label><input className="form-input" value={form.instagram2 ?? ''} onChange={e => set('instagram2', e.target.value)} placeholder="@segundaconta" /></div>
...
function AbaEditoras({ editoras, livrarias, setEditoras, isAdmin, showToast }) {
  ...
  function renderCell(e) {
    return colDefs.map(c => {
      let content = null
      if (c.key === 'classificacao') content = e.classificacao ? <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:24, height:24, borderRadius:6, background:CLASS_COR[e.classificacao]||'var(--accent)', color:'#fff', fontWeight:800, fontSize:12 }}>{e.classificacao}</span> : null
...
function AbaLivrarias({ livrarias, setLivrarias, editoras, isAdmin, showToast }) {
  ...
  function renderCell(l) {
    return colDefs.map(c => {
      let content = null
      if (c.key === 'classificacao') content = l.classificacao ? <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:24, height:24, borderRadius:6, background:CLASS_COR[l.classificacao]||'var(--accent)', color:'#fff', fontWeight:800, fontSize:12 }}>{l.classificacao}</span> : null
      else if (c.key === 'nome') content = l.nome
      ...
      else if (c.key === 'instagram2' && l.instagram2) content = <a href={`https://instagram.com/${l.instagram2.replace('@','')}`} target="_blank" rel="noreferrer" style={{ color:'var(--accent)' }}>{l.instagram2}</a>
