...
// ── FAIXA "PRECISA DE ATENÇÃO" (MELHORADA COM 5W2H) ────────
          ...
          {editandoTitulo?.id === ini.id ? (
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', overflow: 'hidden' }}>
              ...
              <button 
                ...
                style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--accent)', padding: 4, display: 'flex', opacity: 0.7 }}>
                <Info size={13} />
              </button>
            </div>
          )}
    ...
    return (
      <div key={grupo.id}
        ...
        <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12, fontSize: 11 }}>
          <span style={{ color: 'var(--text-muted)' }}>{filhas.length} sub-ações</span>
          {stats.total > 0 && (
            <>
              ...
              {stats.em > 0 && <span style={{ color: '#854F0B', fontWeight: 600 }}>◐ {stats.em}</span>}
    ...
    <div>
      ...
      {iniciativas.length === 0 ? (
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'auto', maxHeight: 'calc(100vh - 220px)' }}>
          {/* CABEÇALHO STICKY — gruda no topo do quadro enquanto a tabela rola por dentro */}
          <div style={{
            display: 'grid',
            gridTemplateColumns: `260px repeat(${semanasDoMes.length}, 1fr)`,
            ...
            {semanasDoMes.map(s => {
              ...
              return (
                <div key={s} style={{ padding: '10px 4px', textAlign: 'center', fontSize: 11, fontWeight: ehAtual ? 700 : 500, color: ehAtual ? 'var(--accent)' : 'var(--text-muted)', borderLeft: '1px solid var(--border)' }}>
  ...
  return (
    <div>
      <div style={{ background: 'var(--surface)', borderRadius: 12, border: '1px solid var(--border)', overflow: 'auto', maxHeight: 'calc(100vh - 220px)' }}>
        ...
        {/* LINHAS */}
        {iniciativas.length === 0 ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 13 }}>Nenhuma iniciativa cadastrada.</div>
        ) : (
          estrutura.map(item => {
            if (item.tipo === 'grupo') {
              return (
  ...
  return (
    <div className="pda-status-report">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {agrupado.map(g => (
          <div key={g.status}>
            <h3 style={{ fontSize: 12, color: g.info.border, textTransform: 'uppercase', marginBottom: 6 }}>{g.info.label}</h3>
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div className="no-print" style={{ display: 'flex', gap: 4, marginBottom: 16, background: 'var(--surface-2)', padding: 4, borderRadius: 8, width: 'fit-content' }}>
        <button onClick={() => setVisao('matriz')} style={{ background: visao === 'matriz' ? 'var(--surface)' : 'transparent', color: visao === 'matriz' ? 'var(--text)' : 'var(--text-muted)', border: 'none', padding: '8px 14px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          <Grid3x3 size={14} /> Matriz mensal
        </button>
        <button onClick={() => setVisao('gantt')} style={{ background: visao === 'gantt' ? 'var(--surface)' : 'transparent', color: visao === 'gantt' ? 'var(--text)' : 'var(--text-muted)', border: 'none', padding: '8px 14px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
          <ChevronRight size={14} /> Linha do tempo
        </button>
        <button onClick={() => setVisao('status')} style={{ background: visao === 'status' ? 'var(--surface)' : 'transparent', color: visao === 'status' ? 'var(--text)' : 'var(--text-muted)', border: 'none', padding: '8px 14px', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div className="no-print" style={{ display: 'flex', gap: 4, marginBottom: 16, borderBottom: '1px solid var(--border)' }}>
        {AREAS.map(a => (
          <button key={a.value} onClick={() => setArea(a.value)} style={{ background: area === a.value ? 'var(--accent)' : 'transparent', color: area === a.value ? 'white' : 'var(--text)', border: 'none', padding: '8px 16px', borderRadius: '8px 8px 0 0', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            {a.label}
          </button>
        ))}
      </div>
      ...
      {toast && <div className="no-print" style={{ position: 'fixed', bottom: 24, right: 24, zIndex: 9999, background: toast.type === 'error' ? 'var(--red)' : 'var(--green)', color: 'white', padding: '10px 16px', borderRadius: 8, fontSize: 13, fontWeight: 600 }}>{toast.msg}</div>}
    </div>
  )
}
