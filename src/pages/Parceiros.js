  ...
  return (
    <div>
      ...
      <div style={{display:'flex',gap:0,marginBottom:20,borderBottom:'1px solid var(--border)'}}>
        {[{v:'parceiros',l:'Parceiros'},{v:'nota_mensal',l:'Nota mensal'}].map(({v,l})=>(
          <button key={v} onClick={()=>setAbaAtiva(v)}
            style={{padding:'10px 20px',background:'none',border:'none',cursor:'pointer',fontSize:13,fontWeight:700,
              color:abaAtiva===v?'var(--accent)':'var(--text-muted)',
              borderBottom:`2px solid ${abaAtiva===v?'var(--accent)':'transparent'}`,
      ...
      {/* Filtros */}
      {abaAtiva==='parceiros'&&<div style={{display:'flex',gap:8,marginBottom:16,flexWrap:'wrap',alignItems:'center'}}>
      ...
      {/* ABA NOTA MENSAL */}
      {abaAtiva==='nota_mensal'&&(
        <ViewNotaMensal parceiros={parceiros}/>
      )}
...
// ── VIEW NOTA MENSAL ──────────────────────────────────────────
function ViewNotaMensal({ parceiros }) {
  const [search, setSearch] = useState('')
  ...
  // Parceiros com pelo menos uma nota mensal
    ...
    <div>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div style={{fontSize:13,color:'var(--text-muted)'}}>
          Nota calculada por mês: média das participações ÷ 4 (meta semanal)
        </div>
                  ...
                  return (
                    <tr key={p.id} style={{borderBottom:'1px solid var(--border)'}}
                      ...
                      <td style={{padding:'10px 14px',textAlign:'center'}}>
                        {geral !== undefined && geral !== null
                          ? <span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:36,height:36,borderRadius:'50%',background:cg.bg,color:cg.cor,fontWeight:800,fontSize:13,border:`2px solid ${cg.cor}40`}}>
                              {geral.toFixed(1)}
                            </span>
                      ...
                      {meses.map(m=>{
                        ...
                        return (
                          <td key={m} style={{padding:'10px 14px',textAlign:'center'}}>
                            {nota !== undefined
                              ? <span style={{display:'inline-flex',alignItems:'center',justifyContent:'center',width:32,height:32,borderRadius:'50%',background:c.bg,color:c.cor,fontWeight:700,fontSize:12,border:`1px solid ${c.cor}40`}}>
                                  {nota.toFixed(1)}
                                </span>
                              : <span style={{color:'var(--text-muted)',fontSize:12}}>—</span>
                            }
