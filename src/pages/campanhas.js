...
export async function getDashboardStatsParceiras({ dataInicio, dataFim } = {}) {

  // Total de editoras parceiras cadastradas
  let qEditoras = supabase.from('editoras_parceiras').select('*', { count: 'exact', head: true })
  if (dataInicio) qEditoras = qEditoras.gte('created_at', dataInicio)
  if (dataFim)    qEditoras = qEditoras.lte('created_at', dataFim + 'T23:59:59')
  const { count: totalEditoras } = await qEditoras

  // Total de tarefas do grupo parceiras
  let qTarefas = supabase.from('tarefas').select('*', { count: 'exact', head: true }).eq('grupo', 'parceiras')
  if (dataInicio) qTarefas = qTarefas.gte('created_at', dataInicio)
  if (dataFim)    qTarefas = qTarefas.lte('created_at', dataFim + 'T23:59:59')
  const { count: totalTarefas } = await qTarefas
  ...
  const tarefasPorStatus = tarefas.reduce((acc, t) => {
    const s = t.status || 'pendente'
    acc[s] = (acc[s] || 0) + 1
    return acc
  }, {})

  // Editoras cadastradas (para exibição)
  const { data: editorasRaw } = await supabase
    .from('editoras_parceiras')
    .select('id, nome, contato')
  const editoras = editorasRaw || []

  // Campanhas do grupo parceiras
  let qCampanhas = supabase.from('campanhas').select('*', { count: 'exact', head: true }).eq('grupo', 'parceiras')
  if (dataInicio) qCampanhas = qCampanhas.gte('data_inicio', dataInicio)
  if (dataFim)    qCampanhas = qCampanhas.lte('data_inicio', dataFim)
  const { count: totalCampanhas } = await qCampanhas
  ...
  const { data: campanhasRaw } = await supabase
    ...
    .eq('grupo', 'parceiras')
  const camps = campanhasRaw || []
  const campanhasPorStatus = camps.reduce((acc, c) => {
    const s = c.status || 'planejamento'
    acc[s] = (acc[s] || 0) + 1
    return acc
  }, {})
  const campanhasPorTipo = camps.reduce((acc, c) => {
