...
function decodificarEtapa(texto) {
  ...
  return {
    texto: String(texto || '').replace(PREFIXO, ''),
...
function normalizarTarefaAntiga(tarefa) {
  const responsaveis = (tarefa.tarefa_responsaveis || []).map(r => ({
    id: r.id,
...
function ResumoCard({ icon: Icon, label, value, tone }) {
  return (
    <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:12, padding:'14px 16px', display:'flex', alignItems:'center', gap:12 }}>
      ...
      <div><div style={{ fontSize:11, color:'var(--text-muted)' }}>{label}</div><div style={{ fontSize:22, fontWeight:800 }}>{value}</div></div>
    </div>
  )
}
  ...
  return (
    <div style={{ background:'var(--surface)', border:`1px solid ${isAtrasada(tarefa) ? 'rgba(239,68,68,.55)' : 'var(--border)'}`, borderRadius:12, padding:16 }}>
      <div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'flex-start' }}>
        <div style={{ minWidth:0 }}>
          <div style={{ display:'flex', gap:7, alignItems:'center', flexWrap:'wrap' }}>
            <div style={{ fontSize:14, fontWeight:700 }}>{tarefa.banco_tarefa?.nome || tarefa.titulo || 'Tarefa'}</div>
            {tarefa._origem === 'antiga' && <span style={{ fontSize:9, color:'var(--text-muted)', border:'1px solid var(--border)', borderRadius:99, padding:'2px 7px', display:'inline-flex', alignItems:'center', gap:3 }}><Archive size={10}/> Antiga</span>}
          </div>
          <div style={{ display:'flex', gap:12, flexWrap:'wrap', fontSize:12, color:'var(--text-muted)', marginTop:5 }}>
            <span style={{ display:'inline-flex', alignItems:'center', gap:4 }}><Building2 size={12}/> {parceiroDa(tarefa)}</span>
            {livro && <span style={{ color:'var(--accent)', display:'inline-flex', alignItems:'center', gap:4 }}><BookOpen size={12}/> {livro}</span>}
            {livrosAntigos.map(l => <span key={l.id} style={{ color:'var(--accent)', display:'inline-flex', alignItems:'center', gap:4 }}><BookOpen size={12}/> {l.titulo}</span>)}
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          <span style={{ fontSize:10, fontWeight:700, color:status.color, background:`${status.color}18`, border:`1px solid ${status.color}45`, borderRadius:99, padding:'3px 8px' }}>{status.label}</span>
          {podeGerenciar && <button className="btn btn-ghost btn-icon btn-sm" onClick={() => onEdit(tarefa)} title="Editar tarefa"><Pencil size={13}/></button>}
          {podeGerenciar && <button className="btn btn-danger btn-icon btn-sm" onClick={() => onDelete(tarefa)} title="Excluir tarefa"><Trash2 size={13}/></button>}
        </div>
      </div>
      ...
      <div style={{ display:'flex', gap:14, flexWrap:'wrap', marginTop:12, fontSize:11, color:'var(--text-muted)' }}>
        ...
        <span>Responsável: <strong>{responsaveis.map(r => r.nome?.split(' ')[0]).join(', ') || 'Não definido'}</strong></span>
        {checklist.length > 0 && <span>Progresso: <strong>{feitos}/{checklist.length} etapas</strong></span>}
      </div>

      {checklist.length > 0 && (
        <div style={{ marginTop:10, padding:'9px 11px', background:'var(--surface-2)', borderRadius:8, display:'grid', gap:5 }}>
          {checklist.slice(0, 4).map((c, i) => <div key={c.id || i} style={{ fontSize:11, color:c.concluido ? '#10b981' : 'var(--text-muted)' }}>{c.concluido ? '✓' : '○'} {c.texto}</div>)}
          {checklist.length > 4 && <div style={{ fontSize:10, color:'var(--text-muted)' }}>+ {checklist.length - 4} etapa(s)</div>}
        </div>
      )}
  ...
  async function salvar() {
    ...
    try {
      ...
      const nova = await atribuirTarefa({
        ...
        atribuidaPor:usuario.id, checklist, parceiroId:form.parceiro_id, quantidade:1,
      })
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal" style={{ maxWidth:680, maxHeight:'92vh', overflowY:'auto' }}>
        <div className="modal-header"><div><h2 className="modal-title">Nova tarefa</h2><p style={{ margin:'3px 0 0', fontSize:11, color:'var(--text-muted)' }}>As datas das etapas serão calculadas pelo prazo final.</p></div><button className="btn btn-ghost btn-icon" onClick={onClose}><X size={16}/></button></div>
        <div className="form-grid">
          <div className="form-group"><label className="form-label">1. Modelo de tarefa *</label><select className="form-select" value={form.modelo_id} onChange={e => setForm(f => ({ ...f, modelo_id:e.target.value }))}><option value="">Selecione</option>{modelos.map(m => <option key={m.id} value={m.id}>{m.nome}</option>)}</select></div>
          <div className="form-row">
            <div className="form-group"><label className="form-label">2. Parceiro ou livraria *</label><select className="form-select" value={form.parceiro_id} onChange={e => setForm(f => ({ ...f, parceiro_id:e.target.value }))}><option value="">Selecione</option>{parceiros.map(p => <option key={p.id} value={p.id}>{p.nome}</option>)}</select></div>
            <div className="form-group"><label className="form-label">3. Livro relacionado *</label><select className="form-select" value={form.livro_id} onChange={e => setForm(f => ({ ...f, livro_id:e.target.value }))}><option value="">Selecione</option>{livros.map(l => <option key={l.id} value={l.id}>{l.titulo}{l.autor ? ` — ${l.autor}` : ''}</option>)}</select></div>
          </div>
          <div className="form-group"><label className="form-label">4. Responsável *</label><div style={{ display:'flex', gap:7, flexWrap:'wrap' }}>{usuarios.map(u => { const ativo = form.responsavel_ids.includes(u.id); return <button key={u.id} type="button" onClick={() => toggle(u.id)} style={{ padding:'5px 12px', borderRadius:20, border:`1px solid ${ativo ? 'var(--accent)' : 'var(--border)'}`, background:ativo ? 'var(--accent-glow)' : 'transparent', color:ativo ? 'var(--accent)' : 'var(--text-muted)', cursor:'pointer' }}>{u.nome.split(' ')[0]}</button> })}</div></div>
          <div className="form-group"><label className="form-label">5. Prazo final</label><input className="form-input" type="date" value={form.data_prazo} onChange={e => setForm(f => ({ ...f, data_prazo:e.target.value }))}/></div>
          {modelo && <div className="form-group"><label className="form-label">Prévia do cronograma</label><div style={{ background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:9, padding:10, display:'grid', gap:7 }}>{etapas.length === 0 ? <span style={{ fontSize:11, color:'var(--text-muted)' }}>Este modelo não tem etapas.</span> : etapas.map((e,i) => <div key={i} style={{ display:'flex', justifyContent:'space-between', gap:10, fontSize:12 }}><span>{i+1}. {e.texto}</span><strong style={{ color:form.data_prazo ? 'var(--accent)' : 'var(--text-muted)' }}>{form.data_prazo ? formatarData(calcularData(form.data_prazo, e.dias_antes)) : (e.dias_antes ? `${e.dias_antes} dias antes` : 'no prazo final')}</strong></div>)}</div></div>}
          <div className="form-group"><label className="form-label">Observação específica</label><textarea className="form-textarea" rows={3} value={form.observacao} onChange={e => setForm(f => ({ ...f, observacao:e.target.value }))}/></div>
        </div>
...
function ModalEditarTarefa({ tarefa, parceiros, livros, usuarios, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    titulo: tarefa.banco_tarefa?.nome || tarefa.titulo || '',
    parceiro_id: tarefa.parceiro?.id || '',
    livro_id: livroAtual?.id || '',
    responsavel_ids: responsaveisDa(tarefa).map(r => r.id),
  ...
  async function salvar() {
    ...
    try {
      if (tarefa._origem === 'antiga') {
        const atualizada = await updateTarefa(tarefa._idOriginal, {
          titulo:form.titulo.trim() || tarefa.titulo,
          descricao:form.observacao.trim() || null,
          data_prazo:form.data_prazo || null,
          status:form.status === 'concluida' ? 'concluido' : form.status,
        })
        ...
        const atualizada = await updateAtribuicao(tarefa.id, {
          parceiro_id:form.parceiro_id || null,
          parceiros_ids:form.parceiro_id ? [form.parceiro_id] : [],
          data_prazo:form.data_prazo || null,
          status:form.status,
          _statusAnterior:tarefa.status,
      ...
      alert('Erro ao editar tarefa: ' + (e.message || 'erro desconhecido'))
    } finally { setSalvando(false) }
  }
  ...
  return (
    <div className="modal-backdrop">
      <div className="modal" style={{ maxWidth:620, maxHeight:'92vh', overflowY:'auto' }}>
        <div className="modal-header"><h2 className="modal-title">Editar tarefa</h2><button className="btn btn-ghost btn-icon" onClick={onClose}><X size={16}/></button></div>
        <div className="form-grid">
          {tarefa._origem === 'antiga' && <div className="form-group"><label className="form-label">Título</label><input className="form-input" value={form.titulo} onChange={e => setForm(f => ({ ...f, titulo:e.target.value }))}/></div>}
          {tarefa._origem !== 'antiga' && <div className="form-row"><div className="form-group"><label className="form-label">Parceiro ou livraria</label><select className="form-select" value={form.parceiro_id} onChange={e => setForm(f => ({ ...f, parceiro_id:e.target.value }))}><option value="">Sem parceiro</option>{parceiros.map(p => <option key={p.id} value={p.id}>{p.nome}</option>)}</select></div><div className="form-group"><label className="form-label">Livro relacionado</label><select className="form-select" value={form.livro_id} onChange={e => setForm(f => ({ ...f, livro_id:e.target.value }))}><option value="">Manter livro atual</option>{livros.map(l => <option key={l.id} value={l.id}>{l.titulo}{l.autor ? ` — ${l.autor}` : ''}</option>)}</select></div></div>}
          <div className="form-group"><label className="form-label">Responsáveis</label><div style={{ display:'flex', flexWrap:'wrap', gap:7 }}>{usuarios.map(u => { const ativo = form.responsavel_ids.includes(u.id); return <button key={u.id} type="button" onClick={() => toggle(u.id)} style={{ padding:'5px 12px', borderRadius:20, border:`1px solid ${ativo ? 'var(--accent)' : 'var(--border)'}`, background:ativo ? 'var(--accent-glow)' : 'transparent', color:ativo ? 'var(--accent)' : 'var(--text-muted)', cursor:'pointer' }}>{u.nome.split(' ')[0]}</button> })}</div></div>
          <div className="form-row"><div className="form-group"><label className="form-label">Prazo</label><input className="form-input" type="date" value={form.data_prazo} onChange={e => setForm(f => ({ ...f, data_prazo:e.target.value }))}/></div><div className="form-group"><label className="form-label">Status</label><select className="form-select" value={form.status} onChange={e => setForm(f => ({ ...f, status:e.target.value }))}>{Object.entries(STATUS).map(([v,s]) => <option key={v} value={v}>{s.label}</option>)}</select></div></div>
          <div className="form-group"><label className="form-label">Observação</label><textarea className="form-textarea" rows={4} value={form.observacao} onChange={e => setForm(f => ({ ...f, observacao:e.target.value }))}/></div>
        </div>
        <div className="form-actions"><button className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary" onClick={salvar} disabled={salvando}>{salvando ? 'Salvando...' : 'Salvar alterações'}</button></div>
      </div>
    </div>
  )
}
...
export default function TarefasInfluencersOrganizadas() {
  ...
  async function carregar() {
    setLoading(true)
    try {
      const [novas, antigas, todosUsuarios, modelosData, parceirosData, livrosRes] = await Promise.all([
        getAtribuicoes({ grupo:'influencers' }), getTarefas(), getUsuarios(), getBancoTarefas('influencers'),
      ...
      const antigasDoGrupo = (antigas || []).filter(t => {
        const ids = [...(t.tarefa_responsaveis || []).map(r => r.usuario_id), t.responsavel?.id, t.responsavel_id].filter(Boolean)
        return ids.some(id => idsEquipe.has(id))
      }).map(normalizarTarefaAntiga)
      setTarefas([...(novas || []).map(t => ({ ...t, _origem:'nova' })), ...antigasDoGrupo])
      setUsuarios(equipe)
      setModelos(modelosData || [])
      setParceiros((parceirosData || []).filter(x => (x.grupo || '') === 'influencers'))
  ...
  const resumo = {
    ...
    concluidas:base.filter(t => t.status === 'concluida' && isSemanaAtual((t.concluida_em || t.updated_at || '').slice(0,10) || t.data_prazo)).length,
  }

  async function mudarStatus(tarefa, status) {
    if (tarefa._origem === 'antiga') {
      const atualizada = await updateTarefa(tarefa._idOriginal, { status:status === 'concluida' ? 'concluido' : status })
      setTarefas(prev => prev.map(x => x.id === tarefa.id ? normalizarTarefaAntiga(atualizada) : x))
      return
    }
    const atualizada = await updateAtribuicao(tarefa.id, { status, _statusAnterior:tarefa.status })
    setTarefas(prev => prev.map(x => x.id === tarefa.id ? { ...atualizada, _origem:'nova' } : x))
  }
  ...
  return (
    <div style={{ padding:'24px 28px 40px', maxWidth:1500, margin:'0 auto' }}>
      ...
      {tarefaEditando && <ModalEditarTarefa tarefa={tarefaEditando} parceiros={parceiros} livros={livros} usuarios={usuarios} onClose={() => setTarefaEditando(null)} onSaved={salva => setTarefas(prev => prev.map(x => x.id === tarefaEditando.id ? salva : x))}/>} 
    </div>
  )
}
