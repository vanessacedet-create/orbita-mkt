...
import { getUsuarios, getParceiros, getLivros, getTarefas, updateTarefa } from '../lib/supabase'
import {
...
function normalizarTarefaAntiga(tarefa) {
  ...
  const livros = (tarefa.tarefa_livros || []).map(x => x.livros).filter(Boolean)
  const status = tarefa.status === 'concluido' ? 'concluida' : tarefa.status
  return {
    ...tarefa,
    _origem: 'antiga',
      ...
      <div style={{ display:'flex', justifyContent:'space-between', gap:12, alignItems:'flex-start' }}>
        <div style={{ minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:7, flexWrap:'wrap', marginBottom:5 }}>
            <div style={{ fontSize:14, fontWeight:700, color:'var(--text)' }}>{tarefa.banco_tarefa?.nome || tarefa.titulo || 'Tarefa'}</div>
            {tarefa._origem === 'antiga' && <span style={{ display:'inline-flex', alignItems:'center', gap:4, fontSize:9, fontWeight:700, color:'var(--text-muted)', background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:99, padding:'2px 7px' }}><Archive size={10}/>Antiga</span>}
          </div>
          <div style={{ display:'flex', gap:12, flexWrap:'wrap', fontSize:12, color:'var(--text-muted)' }}>
            ...
            {livrosAntigos.map(l => <span key={l.id} style={{ display:'inline-flex', alignItems:'center', gap:5, color:'var(--accent)' }}><BookOpen size={12}/>{l.titulo}</span>)}
          </div>
        </div>
    ...
    <div className="modal-backdrop">
      <div className="modal" style={{ maxWidth:620, maxHeight:'92vh', overflowY:'auto' }}>
        ...
        <div className="form-grid">
          <div className="form-group"><label className="form-label">1. Modelo de tarefa *</label><select className="form-select" value={form.modelo_id} onChange={e => setForm(f => ({ ...f, modelo_id:e.target.value }))}><option value="">Selecione o tipo de atividade</option>{modelos.map(m => <option key={m.id} value={m.id}>{m.nome}</option>)}</select>{modelo?.descricao && <div style={{ marginTop:6, padding:'8px 10px', borderRadius:8, background:'var(--surface-2)', color:'var(--text-muted)', fontSize:11 }}>{modelo.descricao}</div>}</div>
          ...
          <div className="form-group"><label className="form-label">4. Responsável *</label><div style={{ display:'flex', flexWrap:'wrap', gap:7 }}>{usuarios.map(u => { const ativo = form.responsavel_ids.includes(u.id); return <button key={u.id} type="button" onClick={() => toggleResponsavel(u.id)} style={{ padding:'5px 12px', borderRadius:20, border:`1px solid ${ativo ? 'var(--accent)' : 'var(--border)'}`, background:ativo ? 'var(--accent-glow)' : 'transparent', color:ativo ? 'var(--accent)' : 'var(--text-muted)', cursor:'pointer', fontSize:12, fontWeight:600 }}>{u.nome.split(' ')[0]}</button> })}</div></div>
          <div className="form-row"><div className="form-group"><label className="form-label">5. Prazo</label><input className="form-input" type="date" value={form.data_prazo} onChange={e => setForm(f => ({ ...f, data_prazo:e.target.value }))}/></div><div className="form-group"><label className="form-label">Etapas puxadas do modelo</label><div style={{ minHeight:38, display:'flex', alignItems:'center', padding:'0 11px', border:'1px solid var(--border)', borderRadius:8, fontSize:12, color:'var(--text-muted)' }}>{modelo ? `${modelo.checklist_padrao?.length || 0} etapa(s)` : 'Selecione um modelo'}</div></div></div>
          <div className="form-group"><label className="form-label">Observação específica</label><textarea className="form-textarea" rows={3} value={form.observacao} onChange={e => setForm(f => ({ ...f, observacao:e.target.value }))} placeholder="Ex.: destacar lançamento, usar cupom, mencionar prazo da campanha..."/></div>
        </div>
...
export default function TarefasInfluencersOrganizadas() {
  ...
  async function carregar() {
    setLoading(true)
    try {
      const [novas, antigas, todosUsuarios, m, p, livrosRes] = await Promise.all([
      ...
      const antigasDoGrupo = (antigas || []).filter(t => {
        const ids = [
          ...(t.tarefa_responsaveis || []).map(r => r.usuario_id),
          t.responsavel?.id,
          t.responsavel_id,
        ].filter(Boolean)
        return ids.some(id => idsEquipe.has(id))
      }).map(normalizarTarefaAntiga)
      const novasNormalizadas = (novas || []).map(t => ({ ...t, _origem:'nova' }))
      setTarefas([...novasNormalizadas, ...antigasDoGrupo])
      setUsuarios(equipe)
      setModelos(m || [])
  ...
  async function mudarStatus(tarefa, status) {
    if (tarefa._origem === 'antiga') {
      const statusAntigo = status === 'concluida' ? 'concluido' : status
      const atualizada = await updateTarefa(tarefa._idOriginal, { status:statusAntigo })
    ...
    setTarefas(prev => prev.map(x => x.id === tarefa.id ? { ...x, ...atualizada, _origem:'nova' } : x))
  }

  if (modelosAberto) {
    return (
      <div style={{ padding:'24px 28px 40px', maxWidth:1600, margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:12, marginBottom:16 }}><div><h1 className="page-title" style={{ margin:0 }}>Modelos de tarefa</h1><p style={{ margin:'5px 0 0', fontSize:12, color:'var(--text-muted)' }}>Cadastre os tipos de atividade, instruções e checklists reutilizáveis.</p></div><button className="btn btn-ghost" onClick={() => { setModelosAberto(false); carregar() }}>Voltar às tarefas</button></div>
  ...
  return (
    <div style={{ padding:'24px 28px 40px', maxWidth:1500, margin:'0 auto' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:16, marginBottom:20, flexWrap:'wrap' }}>
        <div><h1 className="page-title" style={{ margin:0 }}>Tarefas Influencers</h1><p style={{ margin:'5px 0 0', color:'var(--text-muted)', fontSize:12 }}>Tarefas antigas e novas reunidas em uma única visão.</p></div>
        <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}><button className="btn btn-ghost" onClick={carregar}><RefreshCw size={14}/> Atualizar</button>{isAdmin && <button className="btn btn-ghost" onClick={() => setModelosAberto(true)}><Settings size={14}/> Configurar modelos</button>}{isAdmin && <button className="btn btn-primary" onClick={() => setModalNova(true)}><Plus size={14}/> Nova tarefa</button>}</div>
      </div>
