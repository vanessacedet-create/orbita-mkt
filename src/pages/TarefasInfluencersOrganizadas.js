  ...
  async function salvar() {
    ...
    try {
      if (tarefa._origem === 'antiga') {
        const atualizada = await updateTarefa(tarefa._idOriginal, { titulo:form.titulo.trim() || tarefa.titulo, descricao:[`Prioridade: ${form.prioridade}`, form.observacao.trim()].filter(Boolean).join('\n') || null, data_prazo:form.data_prazo || null, status:form.status === 'concluida' ? 'concluido' : form.status })
        onSaved(normalizarTarefaAntiga(atualizada))
      } else {
        const livro = livroSel
        const identificacao = livro ? `${livro.titulo}${livro.autor ? ` — ${livro.autor}` : ''}${livro.isbn ? ` · ISBN ${livro.isbn}` : ''}` : ''
        const atualizada = await updateAtribuicaoInf(tarefa.id, { parceiro_id:form.parceiro_id || null, parceiros_ids:form.parceiro_id ? [form.parceiro_id] : [], data_prazo:form.data_prazo || null, status:form.status, _statusAnterior:tarefa.status, especificidade:[`Prioridade: ${form.prioridade}`, identificacao ? `Livro: ${identificacao}` : '', form.observacao.trim()].filter(Boolean).join('\n'), _responsaveisIds:form.responsavel_ids })
...
export default function TarefasInfluencersOrganizadas() {
  ...
  }).sort(compararPrioridade), [base, filtro, responsavelFiltro])
