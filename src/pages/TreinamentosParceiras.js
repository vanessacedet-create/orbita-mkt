...
async function getColaboradores() {
  const { data } = await supabase
    ...
    .select('id, nome, cargo, gestor_direto, rh_parceiras_grupos(nome)')
    .eq('status', 'ativo')
    .order('nome')
  return data || []
}
...
async function getPlanos() {
  const { data } = await supabase
    .from('planos_parceiras_treinamento')
    .select('*, rh_parceiras_colaboradores(id, nome, cargo, rh_parceiras_grupos(nome))')
...
async function getPlanoCompleto(id) {
  const { data } = await supabase
    .from('planos_parceiras_treinamento')
    .select(`
      *, rh_parceiras_colaboradores(id, nome, cargo, rh_parceiras_grupos(nome)),
      planos_parceiras_secoes(id, titulo, ordem, planos_parceiras_tarefas(id, texto, progresso, observacao, validado_em, ordem)),
      planos_parceiras_historico(id, tipo, descricao, supervisor_nome, secao_titulo, tarefa_texto, created_at),
      planos_parceiras_notificacoes(id, mensagem, lida, created_at)
    `)
    .eq('id', id)
    .single()
  if (!data) return null
  data.planos_parceiras_secoes = (data.planos_parceiras_secoes || []).sort((a, b) => a.ordem - b.ordem)
  data.planos_parceiras_secoes.forEach(s => {
    s.planos_parceiras_tarefas = (s.planos_parceiras_tarefas || []).sort((a, b) => a.ordem - b.ordem)
  })
  data.planos_parceiras_historico = (data.planos_parceiras_historico || []).sort((a, b) =>
    b.created_at.localeCompare(a.created_at))
  data.planos_parceiras_notificacoes = (data.planos_parceiras_notificacoes || []).sort((a, b) =>
    b.created_at.localeCompare(a.created_at))
  return data
}
...
async function deletarPlano(id) {
  await supabase.from('planos_parceiras_treinamento').delete().eq('id', id)
}

async function criarSecao(plano_id, titulo, ordem) {
  const { data } = await supabase
    .from('planos_parceiras_secoes')
    .insert([{ plano_id, titulo, ordem }])
    .select('id, titulo, ordem')
    .single()
  return { ...data, planos_parceiras_tarefas: [] }
}

async function deletarSecao(id) {
  await supabase.from('planos_parceiras_secoes').delete().eq('id', id)
}

async function criarTarefa(secao_id, texto, ordem) {
  const { data } = await supabase
    .from('planos_parceiras_tarefas')
...
async function deletarTarefa(id) {
  await supabase.from('planos_parceiras_tarefas').delete().eq('id', id)
}
...
async function importarTarefasPlanilha(secao_id, tarefas) {
  ...
  const { data } = await supabase.from('planos_parceiras_tarefas').insert(rows).select()
  return data || []
}

async function registrarHistorico(plano_id, tipo, descricao, secao_titulo, tarefa_texto, supervisor_nome) {
  const { data } = await supabase
    .from('planos_parceiras_historico')
...
async function criarNotificacao(plano_id, mensagem) {
  const { data } = await supabase
    .from('planos_parceiras_notificacoes')
...
async function marcarTodasLidas(plano_id) {
  await supabase.from('planos_parceiras_notificacoes').update({ lida: true }).eq('plano_id', plano_id).eq('lida', false)
}
        ...
        <div className="form-grid">
          ...
          {colab && (
            <div style={{ fontSize: 12, color: 'var(--text-muted)', background: 'var(--surface-2)', borderRadius: 8, padding: '8px 12px' }}>
              ...
              {colab.rh_parceiras_grupos?.nome && <> · Grupo: <strong style={{ color: 'var(--accent)' }}>{colab.rh_parceiras_grupos.nome}</strong></>}
            </div>
          )}
          <div className="form-group">
    ...
    <div className="modal-backdrop">
      <div className="modal" style={{ maxWidth: 480 }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--text-muted)', background: 'var(--surface-2)', borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
          Copiando estrutura de <strong style={{ color: 'var(--text)' }}>{plano.rh_parceiras_colaboradores?.nome}</strong> — {plano.planos_parceiras_secoes?.length || 0} seções e todas as tarefas (sem progresso)
        </div>
        <div className="form-grid">
          <div className="form-group">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3 }}>
            <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text)' }}>
              {plano.rh_parceiras_colaboradores?.nome || '—'}
            </span>
            {pct === 100 && (
          ...
          {plano.rh_parceiras_colaboradores?.rh_parceiras_grupos?.nome && (
...
function DetalhePlano({ planoId, supervisorNome, onSupervisorChange, onBack, showToast }) {
  ...
  useEffect(() => {
    getPlanoCompleto(planoId).then(p => {
      setPlano(p)
      if (p?.planos_parceiras_secoes) {
        const exp = {}
        p.planos_parceiras_secoes.forEach(s => { exp[s.id] = true })
        setSecExpanded(exp)
      }
  ...
  const allTasks = plano.planos_parceiras_secoes?.flatMap(s => s.planos_parceiras_tarefas || []) || []
  const total = allTasks.length
  const concluidas = allTasks.filter(t => t.progresso === 100).length
  const pctGeral = total > 0 ? Math.round((concluidas / total) * 100) : 0
  const unread = (plano.planos_parceiras_notificacoes || []).filter(n => !n.lida).length

  function secPct(sec) {
    const ts = sec.planos_parceiras_tarefas || []
  ...
  async function handleStage(tarefa, sec, stage) {
    ...
    setPlano(p => ({
      ...p,
      planos_parceiras_secoes: p.planos_parceiras_secoes.map(s => s.id !== sec.id ? s : {
        ...s,
        planos_parceiras_tarefas: s.planos_parceiras_tarefas.map(t => t.id !== tarefa.id ? t : updated)
      })
    }))
    ...
    setPlano(p => ({ ...p, planos_parceiras_historico: [h, ...(p.planos_parceiras_historico || [])] }))
    if (novo > prev) {
      const n = await criarNotificacao(plano.id, `${supervisorNome} validou "${short(tarefa.texto, 45)}" a ${novo}%`)
      setPlano(p => ({ ...p, planos_parceiras_notificacoes: [n, ...(p.planos_parceiras_notificacoes || [])] }))
    }
  }
  ...
  async function handleSaveNote(tarefa, sec) {
    ...
    if (text) {
      ...
      setPlano(p => ({
        ...p,
        planos_parceiras_historico: [h, ...(p.planos_parceiras_historico || [])],
        planos_parceiras_notificacoes: [n, ...(p.planos_parceiras_notificacoes || [])]
      }))
    }
  ...
  async function handleAddSecao() {
    ...
    const ordem = (plano.planos_parceiras_secoes?.length || 0)
    const sec = await criarSecao(plano.id, novaSecao.trim(), ordem)
    setPlano(p => ({ ...p, planos_parceiras_secoes: [...(p.planos_parceiras_secoes || []), sec] }))
  ...
  async function handleChangeSupervisor() {
    ...
    setPlano(p => ({ ...p, supervisor_nome: newSupervisor.trim(), planos_parceiras_historico: [h, ...(p.planos_parceiras_historico || [])] }))
  ...
  async function handleImportFile(e) {
    ...
    try {
      ...
      const sec = plano.planos_parceiras_secoes.find(s => s.id === importingSecao)
      const ordemBase = sec?.planos_parceiras_tarefas?.length || 0
      const novas = await importarTarefasPlanilha(importingSecao, textos)
      setPlano(p => ({
        ...p,
        planos_parceiras_secoes: p.planos_parceiras_secoes.map(s => s.id !== importingSecao ? s : {
          ...s, planos_parceiras_tarefas: [...(s.planos_parceiras_tarefas || []), ...novas]
        })
      }))
  ...
  async function handleDeleteSecao(sec) {
    ...
    setPlano(p => ({ ...p, planos_parceiras_secoes: p.planos_parceiras_secoes.filter(s => s.id !== sec.id) }))
  ...
  async function handleAddTarefa(sec) {
    ...
    if (!texto) return
    const ordem = (sec.planos_parceiras_tarefas?.length || 0)
    const t = await criarTarefa(sec.id, texto, ordem)
    setPlano(p => ({
      ...p,
      planos_parceiras_secoes: p.planos_parceiras_secoes.map(s => s.id !== sec.id ? s : {
        ...s, planos_parceiras_tarefas: [...(s.planos_parceiras_tarefas || []), t]
      })
    }))
  ...
  async function handleDeleteTarefa(tarefa, sec) {
    ...
    setPlano(p => ({
      ...p,
      planos_parceiras_secoes: p.planos_parceiras_secoes.map(s => s.id !== sec.id ? s : {
        ...s, planos_parceiras_tarefas: s.planos_parceiras_tarefas.filter(t => t.id !== tarefa.id)
      })
    }))
  ...
  async function handleMarcarLidas() {
    ...
    setPlano(p => ({
      ...p,
      planos_parceiras_notificacoes: p.planos_parceiras_notificacoes.map(n => ({ ...n, lida: true }))
    }))
  }

  const barColor = corPorProgresso(pctGeral).color
  const colab = plano.rh_parceiras_colaboradores || {}
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ margin: 0, fontSize: 12, color: 'var(--text-muted)' }}>
            ...
            {colab.rh_parceiras_grupos?.nome && <> · {colab.rh_parceiras_grupos.nome}</>}
          </p>
        </div>
      </div>
      ...
      {notifOpen && (
        <div className="table-card" style={{ marginBottom: 12, overflow: 'hidden' }}>
          ...
          {!(plano.planos_parceiras_notificacoes?.length) ? (
            <div style={{ padding: 20, textAlign: 'center', fontSize: 13, color: 'var(--text-muted)' }}>
              Nenhuma notificação ainda.
            </div>
          ) : plano.planos_parceiras_notificacoes.slice(0, 10).map(n => (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 12 }}>
        {[
          ...
          ['Seções', plano.planos_parceiras_secoes?.length || 0],
        ].map(([l, v]) => (
      ...
      {(plano.planos_parceiras_secoes || []).map(sec => {
        const sp = secPct(sec)
          ...
          <div key={sec.id} className="table-card" style={{ marginBottom: 10, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', borderBottom: isOpen ? '1px solid var(--border)' : 'none' }}>
              <button style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer' }}
                ...
                <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                  {sec.planos_parceiras_tarefas?.length || 0} tarefa{sec.planos_parceiras_tarefas?.length !== 1 ? 's' : ''}
                </span>
              </button>
            ...
            {isOpen && (
              <>
                ...
                {(sec.planos_parceiras_tarefas || []).map((tarefa, ti) => {
                  const isLast = ti === (sec.planos_parceiras_tarefas?.length || 0) - 1
      ...
      <div className="table-card" style={{ overflow: 'hidden' }}>
        <button style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', background: 'none', border: 'none', cursor: 'pointer', borderBottom: histExpanded && plano.planos_parceiras_historico?.length ? '1px solid var(--border)' : 'none' }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {plano.planos_parceiras_historico?.length > 0 && (
              <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                {plano.planos_parceiras_historico.length} registro{plano.planos_parceiras_historico.length !== 1 ? 's' : ''}
              </span>
            )}
          </div>
        ...
        {histExpanded && (
          ...
          ) : plano.planos_parceiras_historico?.map(h => {
...
export default function TreinamentosParceiras() {
  ...
  async function carregar() {
    ...
    const planosComStats = await Promise.all(ps.map(async p => {
      const { data: secoes } = await supabase
        .from('planos_parceiras_secoes')
        .select('planos_parceiras_tarefas(progresso)')
        .eq('plano_id', p.id)
      const tarefas = (secoes || []).flatMap(s => s.planos_parceiras_tarefas || [])
  ...
  async function handleDuplicar(planoOrigem, funcionarioDestino, colabDestino) {
    ...
    for (const sec of (planoCompleto.planos_parceiras_secoes || [])) {
      const novaSec = await criarSecao(novo.id, sec.titulo, sec.ordem)
      if (sec.planos_parceiras_tarefas?.length) {
        const rows = sec.planos_parceiras_tarefas.map(t => ({
          secao_id: novaSec.id,
          texto: t.texto,
          progresso: 0,
          ordem: t.ordem,
        }))
        await supabase.from('planos_parceiras_tarefas').insert(rows)
      }
    }
  ...
  const planosFiltrados = planos.filter(p => {
    ...
    const nome = p.rh_parceiras_colaboradores?.nome?.toLowerCase() || ''
