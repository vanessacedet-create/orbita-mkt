...
function ModalScoreLivrariatMensal({ livraria, score, onSave, onClose }) {
  ...
  const [mes, setMes] = useState(score?.mes || hoje.getMonth()+1)
  const [ano, setAno] = useState(score?.ano || hoje.getFullYear())
  const [form, setForm] = useState({ promocao_geral:score?.promocao_geral||'nao_participou', promocao_particular:score?.promocao_particular||'nao_participou', campanha:score?.campanha||'nao_participou', publicou_feed:score?.publicou_feed||false, publicou_story:score?.publicou_story||false, publicou_reels:score?.publicou_reels||false, vendas_livraria:score?.vendas_livraria||0, responde_artes:score?.responde_artes||false, observacao:score?.observacao||'' })
  const [saving, setSaving] = useState(false)
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{ maxWidth:520, maxHeight:'90vh', overflowY:'auto' }}>
        <div className="modal-header"><div><h2 className="modal-title">Score mensal — Livraria</h2><div style={{ fontSize:12, color:'var(--text-muted)' }}>{livraria.nome}</div></div><button className="btn btn-ghost btn-icon" onClick={onClose}><X size={16} /></button></div>
        <div className="form-group"><label className="form-label">Mês de referência</label><select className="form-select" value={`${ano}-${mes}`} onChange={e => { const [a,m]=e.target.value.split('-'); setAno(Number(a)); setMes(Number(m)) }}>{mesesOpcoes.map(({ mes:m, ano:a }) => <option key={`${a}-${m}`} value={`${a}-${m}`}>{mesAnoLabel(m,a)}</option>)}</select></div>
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12, marginBottom:12 }}>
          {['promocao_geral','promocao_particular','campanha'].map(field => <div key={field} className="form-group"><label className="form-label">{{ promocao_geral:'Promoção geral', promocao_particular:'Promoção particular', campanha:'Campanha' }[field]}</label><select className="form-select" value={form[field]} onChange={e => setForm(f => ({ ...f, [field]:e.target.value }))}>{opcoes.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}</select></div>)}
          <div className="form-group"><label className="form-label">Vendas</label><input className="form-input" type="number" min={0} value={form.vendas_livraria} onChange={e => setForm(f => ({ ...f, vendas_livraria:Number(e.target.value) }))} /></div>
        </div>
        <div style={{ background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:8, padding:'12px 16px', marginBottom:12 }}><div style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', color:'var(--text-muted)', marginBottom:8 }}>Publicações</div><div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:4 }}><Check field="publicou_feed" label="Feed" /><Check field="publicou_story" label="Story" /><Check field="publicou_reels" label="Reels" /><Check field="responde_artes" label="Responde sobre artes" /></div></div>
        <div className="form-group"><label className="form-label">Observação (opcional)</label><textarea className="form-textarea" rows={2} value={form.observacao} onChange={e => setForm(f => ({ ...f, observacao:e.target.value }))} /></div>
        <div style={{ padding:'14px 16px', background:c.bg, border:`1px solid ${c.cor}40`, borderRadius:10, display:'flex', alignItems:'center', gap:14, marginBottom:16 }}><span style={{ fontSize:36, fontWeight:900, color:c.cor }}>{preview.toFixed(1)}</span><div style={{ fontSize:13, fontWeight:700, color:c.cor }}>Score calculado</div></div>
        <div className="form-actions"><button className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary" onClick={salvar} disabled={saving}>{saving?'Salvando...':'Salvar score'}</button></div>
      </div>
