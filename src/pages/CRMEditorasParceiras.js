...
import {
  ...
  deleteScoreEditoraMes, deleteScoreLivrariaMes, deleteScoreLivrariasMes, deleteScoreEditorasMes,
  atualizarClassificacaoMensalLivrarias, getPublicacoesLivrariaMes,
...
// Quadradinho pintado com a cor da classe — usado como primeira coluna na
// Classificação, e também em Editoras & Livrarias e Monitoramento.
function QuadradoClasse({ cls, size = 26 }) {
  if (!cls) return <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:size, height:size, borderRadius:6, background:'var(--surface-2)', border:'1px solid var(--border)', color:'var(--text-muted)', fontSize:12 }}>—</span>
  const cor = corClassificacao(cls)
  return (
    <span style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:size, height:size, borderRadius:6, background:cor, color:'#fff', fontWeight:800, fontSize:size > 24 ? 13 : 12 }}>
      {cls}
    </span>
  )
}
...
function ModalScoreEditoraMensal({ editora, scoresDoEditora = [], mesInicial, anoInicial, onSave, onDelete, onClose }) {
  ...
  function valoresIniciais(s) {
    return {
      vendas_editora: s?.vendas_editora || 0,
      vendas_nao_aplica: s?.vendas_nao_aplica || false,
      comunicacao: s?.comunicacao || 'nao_aplica',
      teve_lancamento: s?.teve_lancamento || false,
      qtd_lancamentos: s?.qtd_lancamentos || 0,
      ...
      <div className="modal" style={{ maxWidth:520, maxHeight:'90vh', overflowY:'auto' }}>
        ...
        <div style={{ background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:8, padding:'14px 16px', marginBottom:12 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            <div style={{ fontSize:12, fontWeight:700, textTransform:'uppercase', color:'var(--text-muted)' }}>Vendas <span style={{ color:'var(--accent)', fontWeight:400 }}>(80%)</span></div>
        ...
        {/* Lançamentos */}
        <div style={{ background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:8, padding:'14px 16px', marginBottom:12 }}>
          <div style={{ fontSize:12, fontWeight:700, textTransform:'uppercase', color:'var(--text-muted)', marginBottom:10 }}>Lançamentos <span style={{ color:'var(--accent)', fontWeight:400 }}>(5%)</span></div>
          <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', marginBottom:8 }}>
            <input type="checkbox" checked={form.teve_lancamento} onChange={e => setForm(f => ({ ...f, teve_lancamento:e.target.checked }))} style={{ accentColor:'var(--accent)', width:15, height:15 }} />
            <span style={{ fontSize:13 }}>Teve lançamento neste mês?</span>
          </label>
          {form.teve_lancamento && (
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span style={{ fontSize:12, color:'var(--text-muted)' }}>Quantos? (informativo, não altera o score)</span>
          ...
          <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:8 }}>Teve pelo menos 1 lançamento → 5% cheios. Não teve → 0%.</div>
        </div>
        ...
        <div style={{ padding:'14px 16px', background:c.bg, border:`1px solid ${c.cor}40`, borderRadius:10, display:'flex', alignItems:'center', gap:14, marginBottom:16 }}>
          ...
          {classePrevia && <QuadradoClasse cls={classePrevia} size={32} />}
          <div>
...
function ModalScoreLivrariatMensal({ livraria, scoresDoLivraria = [], mesInicial, anoInicial, onSave, onDelete, onClose }) {
  const mesesOpcoes = getMesesDisponiveis(24)
  const hoje = new Date()
  const [mes, setMes] = useState(mesInicial || hoje.getMonth()+1)
  const [ano, setAno] = useState(anoInicial || hoje.getFullYear())
  ...
  async function puxarPublicacoes() {
  ...
  return (
    <div className="modal-backdrop" onClick={e => e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{ maxWidth:580, maxHeight:'92vh', overflowY:'auto' }}>
        ...
        <div style={{ background:'var(--surface-2)', border:'1px solid var(--border)', borderRadius:8, padding:'14px 16px', marginBottom:12 }}>
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
            <div style={{ fontSize:12, fontWeight:700, textTransform:'uppercase', color:'var(--text-muted)' }}>Vendas <span style={{ color:'var(--accent)', fontWeight:400 }}>(70%)</span></div>
            <label style={{ display:'flex', alignItems:'center', gap:6, fontSize:12, cursor:'pointer' }}>
              <input type="checkbox" checked={form.vendas_nao_aplica} onChange={e => setForm(f => ({ ...f, vendas_nao_aplica:e.target.checked }))} style={{ accentColor:'var(--accent)' }} />
              Não se aplica
            </label>
          ...
          {!form.vendas_nao_aplica && (
            <div>
              ...
              <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:6 }}>
                Faixas: ≥100 → 70pts · 80-99 → 56pts · 60-79 → 42pts · 40-59 → 28pts · 20-39 → 14pts · 0-19 → 0pts
              </div>
            </div>
          )}
        </div>
...
function ModalZerarMesInteiro({ mes, ano, onConfirm, onClose }) {
  const [zerando, setZerando] = useState(false)

  async function confirmar() {
    if (!window.confirm(`Apagar o score mensal de TODOS os registros em ${mesAnoLabel(mes,ano)}? Essa ação não pode ser desfeita.`)) return
    setZerando(true)
    try { await onConfirm() } finally { setZerando(false) }
  }
    ...
    <div className="modal-backdrop" onClick={e => e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{ maxWidth:420 }}>
        ...
        <p style={{ fontSize:13, color:'var(--text-muted)', marginBottom:14 }}>
          Isso apaga o score de <strong>{mesAnoLabel(mes,ano)}</strong> — o mês que está selecionado na tela agora. Não dá para desfazer.
        </p>
        <div className="form-actions">
          <button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function AbaClassificacao() {
  ...
  // Mês único que comanda tudo: o que aparece na planilha, o que "Atualizar
  // classificação" atualiza, e o que "Zerar mês inteiro" apaga.
  ...
  useEffect(() => { carregar() }, [anoSel, mesSel]) // eslint-disable-line

  useEffect(() => { sessionStorage.setItem('crm_subaba', subAba) }, [subAba])

  // Mapas simples por id — só existe um mês na tela por vez
  ...
  // Ordena pelo score do mês selecionado (o mais alto primeiro); sem score fica no fim
  const editorasFiltradas = editoras
    .filter(e => !search || e.nome.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => (mapEdMensal[b.id]?.score ?? -1) - (mapEdMensal[a.id]?.score ?? -1))
  const livrariasFiltradas = livrarias
    .filter(l => !search || l.nome.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => (mapLivMensal[b.id]?.score ?? -1) - (mapLivMensal[a.id]?.score ?? -1))

  async function handleAtualizarClassificacao() {
    setAtualizando(true)
    try {
      const r = await atualizarClassificacaoMensalLivrarias(anoSel, mesSel)
      let msg = `${r.atualizadas} livraria(s) atualizada(s) em ${mesAnoLabel(mesSel, anoSel)}.`
  ...
  return (
    <div>
      ...
      {/* Mês selecionado + busca + importar + atualizar */}
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16, flexWrap:'wrap', gap:10 }}>
        <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
          <select className="form-select" style={{ width:'auto', fontSize:13, fontWeight:700, padding:'6px 10px', flexShrink:0 }} value={`${anoSel}-${mesSel}`} onChange={e => { const [a,m] = e.target.value.split('-'); setAnoSel(Number(a)); setMesSel(Number(m)) }}>
        ...
        <div style={{ overflowX:'auto' }}>
          <table style={{ width:'100%', borderCollapse:'collapse', maxWidth:640 }}>
            <thead><tr style={{ borderBottom:'1px solid var(--border)' }}>
              <th style={{ padding:'10px 14px', textAlign:'center', fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', width:60 }}>Classe</th>
              <th style={{ padding:'10px 14px', textAlign:'left', fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase' }}>{subAba==='editoras'?'Editora':'Livraria'}</th>
              <th style={{ padding:'10px 14px', textAlign:'center', fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', width:100 }}>Score</th>
            </tr></thead>
            <tbody>
              {subAba === 'editoras' && editorasFiltradas.map(e => {
                ...
                return <tr key={e.id} style={{ borderBottom:'1px solid var(--border)', cursor:'pointer' }} onClick={() => setModalEdMensal({ editora:e })} onMouseEnter={el => el.currentTarget.style.background='var(--surface-2)'} onMouseLeave={el => el.currentTarget.style.background='transparent'}>
                  <td style={{ padding:'10px 14px', textAlign:'center' }}><QuadradoClasse cls={s?.classificacao} /></td>
                  <td style={{ padding:'10px 14px' }}><div style={{ fontWeight:700, fontSize:13, color:'var(--accent)' }}>{e.nome}</div></td>
                  <td style={{ padding:'10px 14px', textAlign:'center' }}><CirculoScore nota={s?.score ?? null} size={32} /></td>
                </tr>
              })}
              {subAba === 'livrarias' && livrariasFiltradas.map(l => {
                ...
                return <tr key={l.id} style={{ borderBottom:'1px solid var(--border)', cursor:'pointer' }} onClick={() => setModalLivMensal({ livraria:l })} onMouseEnter={el => el.currentTarget.style.background='var(--surface-2)'} onMouseLeave={el => el.currentTarget.style.background='transparent'}>
                  <td style={{ padding:'10px 14px', textAlign:'center' }}><QuadradoClasse cls={s?.classificacao} /></td>
                  <td style={{ padding:'10px 14px' }}><div style={{ fontWeight:700, fontSize:13, color:'var(--accent)' }}>{l.nome}</div>{l.editoras_parceiras?.nome && <div style={{ fontSize:11, color:'var(--text-muted)' }}>{l.editoras_parceiras.nome}</div>}</td>
                  <td style={{ padding:'10px 14px', textAlign:'center' }}><CirculoScore nota={s?.score ?? null} size={32} /></td>
                </tr>
              })}
            </tbody>
      ...
      {modalEdMensal && <ModalScoreEditoraMensal editora={modalEdMensal.editora} scoresDoEditora={scoresEdMensal.filter(s => s.editora_id === modalEdMensal.editora.id)}
        mesInicial={mesSel} anoInicial={anoSel}
      ...
      {modalImportar && <ModalImportarVendas tipo={modalImportar} periodo="mensal" onClose={() => setModalImportar(null)} onImported={carregar} showToast={showToast} />}
      {modalZerarMes && (
        <ModalZerarMesInteiro
          mes={mesSel} ano={anoSel}
          onConfirm={async () => {
            const qtd = subAba === 'livrarias' ? await deleteScoreLivrariasMes(anoSel, mesSel) : await deleteScoreEditorasMes(anoSel, mesSel)
            showToast(`${qtd} registro(s) apagado(s) de ${mesAnoLabel(mesSel, anoSel)}.`)
