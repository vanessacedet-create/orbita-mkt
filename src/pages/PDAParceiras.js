...
function CardIniciativa({ ini, subIniciativas, onAbrir }) {
  ...
  if (isComposta) {
    ...
    numAtual = ini.meta_atual || 0
    numTotal = ini.meta_alvo
  } else {
    progresso = progressoTemporal(ini)
  }

  // Pra composta: agrega a menor data de início e a maior data final entre as sub-iniciativas
  let dataTexto
  if (isComposta) {
    const inicios = subIniciativas.map(s => s.data_inicio).filter(Boolean)
    const fins = subIniciativas.map(s => s.prazo_final).filter(Boolean)
    const menorInicio = inicios.length ? inicios.reduce((a, b) => a < b ? a : b) : null
    const maiorFim = fins.length ? fins.reduce((a, b) => a > b ? a : b) : null
    dataTexto = menorInicio && maiorFim ? `${fmtDataBR(menorInicio)} → ${fmtDataBR(maiorFim)}`
      : menorInicio ? `Início: ${fmtDataBR(menorInicio)}`
      : maiorFim ? `Prazo: ${fmtDataBR(maiorFim)}`
      : null
  } else {
    dataTexto = ini.data_inicio && ini.prazo_final
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 16px', transition: 'border-color 0.15s' }}
      onMouseEnter={e => e.currentTarget.style.borderColor = 'var(--accent)'}
      onMouseLeave={e => e.currentTarget.style.borderColor = 'var(--border)'}>
      <div onClick={() => onAbrir(ini)} style={{ cursor: 'pointer' }}>
        ...
        {progresso != null && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
            {numTotal != null && <span style={{ fontSize: 11, color: 'var(--text-muted)', flexShrink: 0 }}>{numAtual}</span>}
            ...
            {numTotal != null && (
              <span style={{ fontSize: 11, color: 'var(--text-muted)', flexShrink: 0, whiteSpace: 'nowrap' }}>
                {numTotal}{!isComposta && ini.meta_unidade ? ` ${ini.meta_unidade}` : ''}
              </span>
            )}
          </div>
        )}
      ...
      {isComposta && subIniciativas.length > 0 && (
        <button onClick={(e) => { e.stopPropagation(); setExpandido(x => !x) }}
          style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 8, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 10, padding: 0 }}>
          <ChevronDown size={12} style={{ transform: expandido ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }} />
          {expandido ? 'Ocultar sub-planejamentos' : 'Ver sub-planejamentos'}
        </button>
      )}

      {isComposta && expandido && (
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {subIniciativas.map(sub => {
            const subSituacao = situacaoIniciativa(sub)
            const subInfo = SITUACAO_INFO[subSituacao]
            const subProgresso = sub.meta_alvo != null ? progressoMeta(sub) : progressoTemporal(sub)
            const subNumAtual = sub.meta_alvo != null ? (sub.meta_atual || 0) : null
            const subNumTotal = sub.meta_alvo != null ? sub.meta_alvo : null
            const subData = sub.data_inicio && sub.prazo_final ? `${fmtDataBR(sub.data_inicio)} → ${fmtDataBR(sub.prazo_final)}`
              : sub.prazo_final ? `Prazo: ${fmtDataBR(sub.prazo_final)}`
              : sub.data_inicio ? `Início: ${fmtDataBR(sub.data_inicio)}`
              : null
            return (
              <div key={sub.id} onClick={(e) => { e.stopPropagation(); onAbrir(sub) }} style={{ cursor: 'pointer' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)' }}>{sub.titulo}</span>
                  <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 6px', borderRadius: 5, background: subInfo.bg, color: subInfo.text, whiteSpace: 'nowrap' }}>{subInfo.label}</span>
                </div>
                {subProgresso != null && (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, margin: '4px 0 3px' }}>
                    {subNumTotal != null && <span style={{ fontSize: 10, color: 'var(--text-muted)', flexShrink: 0 }}>{subNumAtual}</span>}
                    <div style={{ flex: 1, height: 4, background: 'var(--surface-2)', borderRadius: 99, overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${subProgresso}%`, background: subInfo.fill }} />
                    </div>
                    {subNumTotal != null && (
                      <span style={{ fontSize: 10, color: 'var(--text-muted)', flexShrink: 0 }}>
                        {subNumTotal}{sub.meta_unidade ? ` ${sub.meta_unidade}` : ''}
                      </span>
                    )}
                  </div>
                )}
                {subData && <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>{subData}</div>}
              </div>
            )
          })}
        </div>
