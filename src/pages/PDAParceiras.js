    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" style={{ maxWidth: 520, padding: 22 }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div>
            <strong style={rotuloStyle}>O que fazer (What)</strong>
            <input className="form-input" style={{ ...campoStyle, fontWeight: 600 }}
              value={titulo}
              onChange={e => setTitulo(e.target.value)}
              onBlur={() => onSalvarCampo(iniciativa.id, 'titulo', titulo.trim() || iniciativa.titulo)}
              onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur() }}
              placeholder="Título da iniciativa" />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
            <div>
              <strong style={rotuloStyle}>Quem (Who)</strong>
              <input className="form-input" style={campoStyle}
                value={responsavel}
                onChange={e => setResponsavel(e.target.value)}
                onBlur={() => onSalvarCampo(iniciativa.id, 'responsavel', responsavel.trim() || null)}
                onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur() }}
                placeholder="Responsável" />
            </div>
            <div>
              <strong style={rotuloStyle}>Por que (Why)</strong>
              <input className="form-input" style={campoStyle}
                value={justificativa}
                onChange={e => setJustificativa(e.target.value)}
                onBlur={() => onSalvarCampo(iniciativa.id, 'justificativa', justificativa.trim() || null)}
                onKeyDown={e => { if (e.key === 'Enter') e.currentTarget.blur() }}
                placeholder="Objetivo / motivo" />
            </div>
          </div>
          <div>
            <strong style={rotuloStyle}>Como fazer (How) — Roteiro de Etapas</strong>
            <textarea className="form-input" rows={3} style={{ ...campoStyle, resize: 'vertical', lineHeight: 1.4 }}
              value={comoFazer}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, borderTop: '1px solid var(--border)', paddingTop: 14 }}>
            <div>
              <strong style={rotuloStyle}>Onde (Where)</strong>
              <p style={{ margin: '8px 0 0 0', fontSize: 13, color: 'var(--text-muted)' }}>Plataforma Órbita MKT</p>
            </div>
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginTop: 6 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <input type="date" className="form-input" style={{ fontSize: 12, padding: '4px 6px', cursor: 'pointer' }}
                    ...
                    onBlur={() => onSalvarCampo(iniciativa.id, 'data_inicio', dataInicio || null)}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <input type="date" className="form-input" style={{ fontSize: 12, padding: '4px 6px', color: 'var(--accent)', cursor: 'pointer' }}
                    ...
                    onBlur={() => onSalvarCampo(iniciativa.id, 'prazo_final', prazoFinal || null)}
                    onClick={e => e.currentTarget.showPicker?.()} />
                </div>
              </div>
            </div>
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              <strong style={{ ...rotuloStyle, display: 'flex', alignItems: 'center', gap: 6 }}>
                <ListChecks size={13} /> Subtarefas
              </strong>
              <button className="btn btn-ghost btn-sm" style={{ fontSize: 11 }} onClick={() => onAbrirSubtarefas(iniciativa)}>Gerenciar subtarefas</button>
            </div>
            {(iniciativa.pda_parceiras_secoes || []).length === 0 ? (
              <p style={{ fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic', margin: 0 }}>Nenhuma subtarefa ainda.</p>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {iniciativa.pda_parceiras_secoes.slice().sort((a, b) => (a.ordem || 0) - (b.ordem || 0)).map(sec => {
...
export default function PDAParceiras() {
  ...
  const [detalheIniId, setDetalheIniId] = useState(null)
  ...
  async function handleSalvarCampoDetalhe(iniciativaId, campo, valor) {
    try {
      const upd = await atualizarIniciativa(iniciativaId, { [campo]: valor })
    ...
    <div style={{ padding: 24 }}>
      ...
      {!loading && iniciativasVisiveis.length > 0 && (
        <div className="no-print">
          ...
          {pdaSituacao === 'ativos' && (
            <PdaAtencao iniciativas={iniciativasVisiveis} semanaAtualIdx={semanaAtualIdx} onVerDetalhes={(ini) => setDetalheIniId(ini.id)} />
          )}
        </div>
      )}
      ...
      {loading ? (
        ...
        <VisaoGantt
          ...
          onVerDetalhes={(ini) => setDetalheIniId(ini.id)}
        />
      ) : (
        <VisaoStatusReport
      ...
      {modalSemestre && <ModalNovoSemestre onSave={handleCriarSemestre} onClose={() => setModalSemestre(false)} />}
      {detalheIniId && (() => {
        const iniDetalhe = iniciativas.find(i => i.id === detalheIniId)
        if (!iniDetalhe) return null
        return (
          <ModalDetalhes5W2H
