...
function Shell() {
  ...
  const menuVisivel=MENU.filter(m=>{if(ehDono&&!viewAs&&['/campanhas','/promocoes-parceiras'].includes(m.path))return false;if(m.soEmail&&!ehDono)return false;if(m.ocultarPerfis&&m.ocultarPerfis.includes(perfilAtivo))return false;return m.sempreVisivel||canAtivo(m.modulo)}).filter((m,i,a)=>a.findIndex(x=>x.path===m.path)===i)
